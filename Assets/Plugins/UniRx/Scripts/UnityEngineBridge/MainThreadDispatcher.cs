...
namespace UniRx
{
    public sealed class MainThreadDispatcher : MonoBehaviour
    {
        ...
        {
            private static object gate = new();
            ...
            public static EditorThreadDispatcher Instance
            {
                get
                {
                    ...
                    lock (gate)
                    {
                        if (instance == null) instance = new EditorThreadDispatcher();
            ...
            private ThreadSafeQueueWorker editorQueueWorker = new();
...
#pragma warning restore CS0618
        ...
        public static void Post(Action<object> action, object state)
        {
            ...
            if (!isQuitting && !ReferenceEquals(dispatcher, null)) dispatcher.queueWorker.Enqueue(action, state);
        }
        ...
        public static void Send(Action<object> action, object state)
        {
            ...
            if (mainThreadToken != null)
                ...
                {
                    var dispatcher = Instance;
                    if (dispatcher != null) dispatcher.unhandledExceptionCallback(ex);
                }
            else
                Post(action, state);
        }
            ...
            else
            {
                ...
                if (!isQuitting && !ReferenceEquals(dispatcher, null))
                    dispatcher.queueWorker.Enqueue(_ =>
                    {
                        ...
                        if (dispacher2 != null) (dispacher2 as MonoBehaviour).StartCoroutine(routine);
                    }, null);
            }
        }
        ...
        public static void StartUpdateMicroCoroutine(IEnumerator routine)
        {
            ...
            var dispatcher = Instance;
            if (dispatcher != null) dispatcher.updateMicroCoroutine.AddCoroutine(routine);
        }
        ...
        public static void StartFixedUpdateMicroCoroutine(IEnumerator routine)
        {
            ...
            var dispatcher = Instance;
            if (dispatcher != null) dispatcher.fixedUpdateMicroCoroutine.AddCoroutine(routine);
        }
        ...
        public static void StartEndOfFrameMicroCoroutine(IEnumerator routine)
        {
            ...
            var dispatcher = Instance;
            if (dispatcher != null) dispatcher.endOfFrameMicroCoroutine.AddCoroutine(routine);
        }
        ...
        private ThreadSafeQueueWorker queueWorker = new();
        ...
        public static string InstanceName
        {
            get
            {
                if (instance == null) throw new NullReferenceException("MainThreadDispatcher is not initialized.");
        ...
        public static void CullAllExcessDispatchers()
        {
            ...
            for (var i = 0; i < dispatchers.Length; i++) DestroyDispatcher(dispatchers[i]);
        }
