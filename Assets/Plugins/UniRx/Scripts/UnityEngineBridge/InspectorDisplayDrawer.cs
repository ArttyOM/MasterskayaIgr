...
namespace UniRx
{
    ...
    public class InspectorDisplayDrawer : PropertyDrawer
    {
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            if (notifyPropertyChanged) EditorGUI.BeginChangeCheck();
            ...
            if (targetSerializedProperty == null)
            {
                ...
                if (notifyPropertyChanged) EditorGUI.EndChangeCheck();
                return;
            }
            else
            {
            ...
            if (notifyPropertyChanged)
            {
                if (EditorGUI.EndChangeCheck())
                {
                    ...
                    var methodInfo = targetProp.GetType().GetMethod("SetValueAndForceNotify",
                        ...
                        BindingFlags.Public | BindingFlags.NonPublic);
                    if (methodInfo != null) methodInfo.Invoke(targetProp, new object[] {modifiedValue});
                }
                else
                {
        ...
        private object GetValueRecursive(object obj, int index, string[] paths)
        {
            ...
            if (index < paths.Length - 1) return GetValueRecursive(v, ++index, paths);
        ...
        public override float GetPropertyHeight(SerializedProperty property, GUIContent label)
        {
            ...
            if (valueProperty == null) return height;

            if (valueProperty.propertyType == SerializedPropertyType.Rect) return height * 2;
            if (valueProperty.propertyType == SerializedPropertyType.Bounds) return height * 3;
