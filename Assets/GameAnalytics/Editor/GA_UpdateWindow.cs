...
namespace GameAnalyticsSDK.Editor
{
    ...
    {
        private GUIContent _close = new("Skip", "Skip this version.");

        private GUIContent _download = new("Download Page", "Open the GameAnalytics download support page.");
        ...
        private void OnGUI()
        {
            ...
            GUILayout.Label("You can download the latest version from the GameAnalytics download support page.",
                EditorStyles.wordWrappedMiniLabel, GUILayout.MaxWidth(320));
            ...
            {
                EditorPrefs.SetString("ga_skip_version" + "-" + Application.dataPath,
                    GameAnalytics.SettingsGA.NewVersion);
                Close();
            }
        ...
        {
            if (!string.IsNullOrEmpty(newVersion)) GameAnalytics.SettingsGA.NewVersion = newVersion;
        }
        ...
        {
            if (!string.IsNullOrEmpty(changes)) GameAnalytics.SettingsGA.Changes = changes;
        }
        ...
        public static string UpdateStatus(string currentVersion)
        {
            try
            {
                var newV = int.Parse(GameAnalytics.SettingsGA.NewVersion.Replace(".", ""));
                var oldV = int.Parse(currentVersion.Replace(".", ""));
