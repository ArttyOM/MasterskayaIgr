...
namespace GameAnalyticsSDK.Editor
{
    ...
    public class GA_SettingsInspector : UnityEditor.Editor
    {
        ...
        private const string AssetsPrependPath =
            IsCustomPackage ? "Packages/com.gameanalytics.sdk" : "Assets/GameAnalytics";

        private GUIContent _publicKeyLabel =
            new("Game Key", "Your GameAnalytics Game Key - copy/paste from the GA website.");

        private GUIContent _privateKeyLabel =
            new("Secret Key", "Your GameAnalytics Secret Key - copy/paste from the GA website.");

        private GUIContent _emailLabel = new("Email", "Your GameAnalytics user account email.");

        private GUIContent _passwordLabel = new("Password",
            "Your GameAnalytics user account password. Must be at least 8 characters in length.");

        private GUIContent _organizationsLabel = new("Org.", "Organizations tied to your GameAnalytics user account.");
        private GUIContent _studiosLabel = new("Studio", "Studios tied to your GameAnalytics user account.");
        private GUIContent _gamesLabel = new("Game", "Games tied to the selected GameAnalytics studio.");

        private GUIContent _build = new("Build",
            "The current version of the game. Updating the build name for each test version of the game will allow you to filter by build when viewing your data on the GA website.");

        private GUIContent _infoLogEditor = new("Info Log Editor",
            "Show info messages from GA in the unity editor console when submitting data.");

        private GUIContent _infoLogBuild =
            new("Info Log Build", "Show info messages from GA in builds (f.x. Xcode for iOS).");

        private GUIContent _verboseLogBuild = new("Verbose Log Build",
            "Show full info messages from GA in builds (f.x. Xcode for iOS). Noet that this option includes long JSON messages sent to the server.");

        private GUIContent _useManualSessionHandling = new("Use manual session handling",
        ...
        private GUIContent _usePlayerSettingsBunldeVersionForBuild = new("Send Version* (Android, iOS) as build number",
            "The SDK will automatically fetch the version* number on Android and iOS and send it as the GameAnalytics build number.");
#else
        private GUIContent _usePlayerSettingsBunldeVersionForBuild =
 new GUIContent("Send Build number (iOS) and Version* (Android) as build number", "The SDK will automatically fetch the build number on iOS and the version* number on Android and send it as the GameAnalytics build number.");
#endif
        ...
        private GUIContent _account = new("Account",
            "This tab allows you to easily create a GameAnalytics account. You can also login to automatically retrieve your Game Key and Secret Key.");

        private GUIContent _setup = new("Setup",
            "This tab shows general options which are relevant for a wide variety of messages sent to GameAnalytics.");

        private GUIContent _advanced =
            new("Advanced", "This tab shows advanced and misc. options for the GameAnalytics SDK.");

        private GUIContent _customDimensions01 = new("Custom Dimensions 01", "List of custom dimensions 01.");
        private GUIContent _customDimensions02 = new("Custom Dimensions 02", "List of custom dimensions 02.");
        private GUIContent _customDimensions03 = new("Custom Dimensions 03", "List of custom dimensions 03.");
        private GUIContent _resourceItemTypes = new("Resource Item Types", "List of Resource Item Types.");
        private GUIContent _resourceCurrrencies = new("Resource Currencies", "List of Resource Currencies.");
        private GUIContent _gaFpsAverage = new("Submit Average FPS", "Submit the average frames per second.");

        private GUIContent _gaFpsCritical = new("Submit Critical FPS",
            "Submit a message whenever the frames per second falls below a certain threshold. The location of the Track Target will be used for critical FPS events.");

        private GUIContent _gaFpsCriticalThreshold = new("FPS <", "Frames per second threshold.");

        private GUIContent _gaSubmitErrors = new("Submit Errors",
        ...
        private GUIContent _gaNativeErrorReporting = new("Native error reporting (Android, iOS)",
            "Submit error and exception messages from native errors and exceptions to the GameAnalytics server. Useful for getting relevant data when the game crashes, etc. from native code.");

        private GUIContent _gameSetupIcon;
        private bool _gameSetupIconOpen = false;

        private GUIContent _gameSetupIconMsg =
            new(
                "Your game and secret key will authenticate the game. Please set the build version too. All fields are required.");

        private GUIContent _customDimensionsIcon;
        private bool _customDimensionsIconOpen = false;

        private GUIContent _customDimensionsIconMsg =
            new("Define your custom dimension values below. Values that are not defined will be ignored.");

        private GUIContent _resourceTypesIcon;
        private bool _resourceTypesIconOpen = false;

        private GUIContent _resourceTypesIconMsg =
            new(
                "Define all your resource currencies and resource item types. Values that are not defined will be ignored.");

        private GUIContent _advancedSettingsIcon;
        private bool _advancedSettingsIconOpen = false;

        private GUIContent _advancedSettingsIconMsg =
            new(
                "Advanced settings allows you to enable tracking of Unity errors and exceptions, and frames per second (for performance).");

        private GUIContent _debugSettingsIcon;
        private bool _debugSettingsIconOpen = false;

        private GUIContent _debugSettingsIconMsg =
            new(
                "Debug settings allows you to enable info log for the editor or for builds (Xcode, etc.). Enabling verbose logging will show additional JSON messages in builds.");
        ...
        {
            var ga = target as Settings;

            if (ga.UpdateIcon == null)
                ga.UpdateIcon = (Texture2D) AssetDatabase.LoadAssetAtPath(
                    AssetsPrependPath + "/Gizmos/GameAnalytics/Images/update_orange.png", typeof(Texture2D));

            if (ga.DeleteIcon == null)
                ga.DeleteIcon =
                    (Texture2D) AssetDatabase.LoadAssetAtPath(
                        AssetsPrependPath + "/Gizmos/GameAnalytics/Images/delete.png", typeof(Texture2D));

            if (ga.GameIcon == null)
                ga.GameIcon =
                    (Texture2D) AssetDatabase.LoadAssetAtPath(
                        AssetsPrependPath + "/Gizmos/GameAnalytics/Images/game.png", typeof(Texture2D));

            if (ga.HomeIcon == null)
                ga.HomeIcon =
                    (Texture2D) AssetDatabase.LoadAssetAtPath(
                        AssetsPrependPath + "/Gizmos/GameAnalytics/Images/home.png", typeof(Texture2D));

            if (ga.InfoIcon == null)
                ga.InfoIcon =
                    (Texture2D) AssetDatabase.LoadAssetAtPath(
                        AssetsPrependPath + "/Gizmos/GameAnalytics/Images/info.png", typeof(Texture2D));

            if (ga.InstrumentIcon == null)
                ga.InstrumentIcon = (Texture2D) AssetDatabase.LoadAssetAtPath(
                    AssetsPrependPath + "/Gizmos/GameAnalytics/Images/instrument.png", typeof(Texture2D));

            if (ga.QuestionIcon == null)
                ga.QuestionIcon =
                    (Texture2D) AssetDatabase.LoadAssetAtPath(
                        AssetsPrependPath + "/Gizmos/GameAnalytics/Images/question.png", typeof(Texture2D));

            if (ga.UserIcon == null)
                ga.UserIcon =
                    (Texture2D) AssetDatabase.LoadAssetAtPath(
                        AssetsPrependPath + "/Gizmos/GameAnalytics/Images/user.png", typeof(Texture2D));

            if (_gameSetupIcon == null) _gameSetupIcon = new GUIContent(ga.InfoIcon, "Game Setup.");

            if (_customDimensionsIcon == null)
                _customDimensionsIcon = new GUIContent(ga.InfoIcon, "Custom Dimensions.");

            if (_resourceTypesIcon == null) _resourceTypesIcon = new GUIContent(ga.InfoIcon, "Resource Types.");

            if (_advancedSettingsIcon == null)
                _advancedSettingsIcon = new GUIContent(ga.InfoIcon, "Advanced Settings.");

            if (_debugSettingsIcon == null) _debugSettingsIcon = new GUIContent(ga.InfoIcon, "Debug Settings.");

            if (_deleteIcon == null) _deleteIcon = new GUIContent(ga.DeleteIcon, "Delete.");

            if (_homeIcon == null) _homeIcon = new GUIContent(ga.HomeIcon, "Your GameAnalytics webpage tool.");

            if (_instrumentIcon == null)
                _instrumentIcon = new GUIContent(ga.InstrumentIcon, "GameAnalytics setup guide.");

            if (_questionIcon == null) _questionIcon = new GUIContent(ga.QuestionIcon, "GameAnalytics support.");

            if (ga.Logo == null)
                ga.Logo = (Texture2D) AssetDatabase.LoadAssetAtPath(
                    AssetsPrependPath + "/Gizmos/GameAnalytics/gaLogo.png", typeof(Texture2D));
        }
        ...
        public override void OnInspectorGUI()
        {
            ...
            {
                var signupButton = new GUIStyle(GUI.skin.button);
                signupButton.normal.background =
                    (Texture2D) AssetDatabase.LoadAssetAtPath(
                        AssetsPrependPath + "/Gizmos/GameAnalytics/Images/default.png", typeof(Texture2D));
                signupButton.active.background =
                    (Texture2D) AssetDatabase.LoadAssetAtPath(
                        AssetsPrependPath + "/Gizmos/GameAnalytics/Images/active.png", typeof(Texture2D));
            ...
            DrawLinkButton(_homeIcon, GUI.skin.label, "https://go.gameanalytics.com/login", GUILayout.Width(24),
                GUILayout.Height(24));

            DrawLinkButton(_questionIcon, GUI.skin.label, "http://support.gameanalytics.com/", GUILayout.Width(24),
                GUILayout.Height(24));

            DrawButton(_instrumentIcon, GUI.skin.label, OpenSignUpSwitchToGuideStep, GUILayout.Width(24),
                GUILayout.Height(24));
            ...
            var updateStatus = GA_UpdateWindow.UpdateStatus(Settings.VERSION);
            ...
            if (!updateStatus.Equals(string.Empty))
            {
                ...
                if (GUILayout.Button(ga.UpdateIcon, _orangeUpdateIconStyle, GUILayout.MaxWidth(17))) OpenUpdateWindow();
            ...
            if (ga.Organizations != null)
            {
                ...
                float minW = 0;
                float maxW = 0;
                var email = new GUIContent(ga.EmailGA);
            ...
            if (ga.IntroScreen)
            {
                ...
                for (var i = 0; i < GameAnalytics.SettingsGA.Platforms.Count; ++i)
                    if (GameAnalytics.SettingsGA.GetGameKey(i).Length > 0 ||
                        GameAnalytics.SettingsGA.GetSecretKey(i).Length > 0)
                    {
                        finishIntro = true;
                        break;
                    }
                ...
                {
                    if (!_checkedProjectNames &&
                        !EditorPrefs.GetBool("GA_Installed" + "-" + Application.dataPath, false))
                    {
                    ...
                    var largeWhiteStyle = new GUIStyle(EditorStyles.whiteLargeLabel);
                    if (!Application.HasProLicense()) largeWhiteStyle = new GUIStyle(EditorStyles.largeLabel);
                    ...
                    var greyStyle = new GUIStyle(EditorStyles.label);
                    ...
                    DrawButtonWithFlexibleSpace("Sign up", ga.SignupButton, OpenSignUp, GUILayout.Width(175),
                        GUILayout.Height(40));
                    ...
                    DrawLinkButton("Forgot password?", EditorStyles.label,
                        "https://go.gameanalytics.com/login?showreset&email=" + ga.EmailGA, GUILayout.Width(105));
                    ...
                    GUILayout.BeginHorizontal();
                    GUILayout.FlexibleSpace();
                    if (GUILayout.Button("I want to fill in my game keys manually", EditorStyles.label,
                            GUILayout.Width(207)))
                    {
                        ga.IntroScreen = false;
            ...
            else
            {
                ...
                var activeTabStyle = new GUIStyle(EditorStyles.miniButtonMid);
                var activeTabStyleLeft = new GUIStyle(EditorStyles.miniButtonLeft);
                var activeTabStyleRight = new GUIStyle(EditorStyles.miniButtonRight);
                ...
                var inactiveTabStyle = new GUIStyle(EditorStyles.miniButtonMid);
                var inactiveTabStyleLeft = new GUIStyle(EditorStyles.miniButtonLeft);
                var inactiveTabStyleRight = new GUIStyle(EditorStyles.miniButtonRight);

                var basicTabStyle = ga.CurrentInspectorState == Settings.InspectorStates.Basic
                    ? activeTabStyleLeft
                    : inactiveTabStyleLeft;

                if (ga.Organizations == null)
                {
                    if (GUILayout.Button(_account,
                            ga.CurrentInspectorState == Settings.InspectorStates.Account
                                ? activeTabStyleLeft
                                : inactiveTabStyleLeft)) ga.CurrentInspectorState = Settings.InspectorStates.Account;

                    basicTabStyle = ga.CurrentInspectorState == Settings.InspectorStates.Basic
                        ? activeTabStyle
                        : inactiveTabStyle;
                }

                if (GUILayout.Button(_setup, basicTabStyle)) ga.CurrentInspectorState = Settings.InspectorStates.Basic;

                if (GUILayout.Button(_advanced,
                        ga.CurrentInspectorState == Settings.InspectorStates.Pref
                            ? activeTabStyleRight
                            : inactiveTabStyleRight)) ga.CurrentInspectorState = Settings.InspectorStates.Pref;
                ...
                if (ga.CurrentInspectorState == Settings.InspectorStates.Account)
                {
                    ...
                    if (!string.IsNullOrEmpty(ga.LoginStatus) && !ga.LoginStatus.Equals("Not logged in."))
                    {
                        ...
                        if (ga.JustSignedUp && !ga.HideSignupWarning)
                        {
                            ...
                            EditorGUILayout.HelpBox(
                                "Please be aware that our service might take a few minutes to get ready to receive events. Click here to open Integration Status to follow the progress as you start sending events.",
                                MessageType.Warning);
                            ...
                            if (GUILayout.Button("X")) ga.HideSignupWarning = true;
                            GUILayout.EndHorizontal();
                            EditorGUILayout.Space();
                        }
                    ...
                    if (ga.Organizations == null)
                    {
                        ...
                        {
                            var signup = CreateInstance<GA_SignUp>();
                        ...
                        GUILayout.FlexibleSpace();
                        if (GUILayout.Button("I want to fill in my game keys manually", EditorStyles.label,
                                GUILayout.Width(207))) ga.CurrentInspectorState = Settings.InspectorStates.Basic;
                        EditorGUIUtility.AddCursorRect(GUILayoutUtility.GetLastRect(), MouseCursor.Link);
                        GUILayout.FlexibleSpace();
                        GUILayout.EndHorizontal();
                    }
                ...
                else if (ga.CurrentInspectorState == Settings.InspectorStates.Basic)
                {
                    ...
                    if (!_gameSetupIconOpen) GUI.color = new Color(0.54f, 0.54f, 0.54f);
                    ...
                    {
                        GUILayout.BeginHorizontal();
                        var tmpAnchor = GUI.skin.box.alignment;
                        GUI.skin.box.alignment = TextAnchor.UpperLeft;
                        var tmpColor = GUI.skin.box.normal.textColor;
                        GUI.skin.box.normal.textColor = new Color(0.7f, 0.7f, 0.7f);
                        var tmpOffset = GUI.skin.box.padding;
                    ...
                    {
                        var diff = ga.SelectedPlatformOrganization.Count - GameAnalytics.SettingsGA.Platforms.Count;

                        if (diff < 0)
                        {
                            var absDiff = Mathf.Abs(diff);

                            for (var i = 0; i < absDiff; ++i) ga.SelectedPlatformOrganization.Add("");
                        }
                        else
                        {
                    ...
                    for (var i = 0; i < GameAnalytics.SettingsGA.Platforms.Count; ++i)
                    {
                        ga.PlatformFoldOut[i] = EditorGUILayout.Foldout(ga.PlatformFoldOut[i],
                            PlatformToString(GameAnalytics.SettingsGA.Platforms[i]));

                        if (ga.PlatformFoldOut[i])
                        {
                            if (ga.Organizations != null && ga.Organizations.Count > 0 &&
                                i < ga.SelectedOrganization.Count)
                            {
                                EditorGUILayout.Space();
                                //Splitter(new Color(0.35f, 0.35f, 0.35f));
                                ...
                                var organizationNames = Organization.GetOrganizationNames(ga.Organizations);
                                if (ga.SelectedOrganization[i] >= organizationNames.Length)
                                    ga.SelectedOrganization[i] = 0;
                                var tmpSelectedOrganization = ga.SelectedOrganization[i];
                                ga.SelectedOrganization[i] =
                                    EditorGUILayout.Popup("", ga.SelectedOrganization[i], organizationNames);
                                ...
                                if (ga.SelectedOrganization[i] > 0)
                                {
                                    ...
                                    var studioNames =
                                        Studio.GetStudioNames(ga.Organizations[ga.SelectedOrganization[i] - 1].Studios);
                                    if (ga.SelectedStudio[i] >= studioNames.Length) ga.SelectedStudio[i] = 0;
                                    var tmpSelectedStudio = ga.SelectedStudio[i];
                                    ...
                                    if (ga.SelectedStudio[i] > 0)
                                    {
                                        ...
                                        var gameNames = Studio.GetGameNames(ga.SelectedStudio[i] - 1,
                                            ga.Organizations[ga.SelectedOrganization[i] - 1].Studios);
                                        if (ga.SelectedGame[i] >= gameNames.Length) ga.SelectedGame[i] = 0;

                                        var tmpSelectedGame = ga.SelectedGame[i];
                            ...
                            else
                            {
                                ...
                                GUILayout.Space(-10);
                                GUILayout.Label(!string.IsNullOrEmpty(ga.SelectedPlatformOrganization[i])
                                ...
                                GUILayout.Space(-10);
                                GUILayout.Label(!string.IsNullOrEmpty(ga.SelectedPlatformStudio[i])
                                ...
                                GUILayout.Space(-10);
                                GUILayout.Label(!string.IsNullOrEmpty(ga.SelectedPlatformGame[i])
                            ...
                            GUILayout.Space(-10);
                            var beforeGameKey = ga.GetGameKey(i);
                            var tmpGameKey = EditorGUILayout.TextField("", ga.GetGameKey(i));
                            ...
                            GUILayout.Space(-10);
                            var beforeSecretKey = ga.GetSecretKey(i);
                            var tmpSecretKey = EditorGUILayout.TextField("", ga.GetSecretKey(i));
                                ...
                                case true:
                                    ...
                                    else
                                    {
                                        if (GameAnalytics.SettingsGA.Platforms[i] == RuntimePlatform.Android)
                                        {
                                            ...
                                            EditorGUILayout.HelpBox(
                                                "Using Android Player Settings Version* number as build number in events. \nBuild number is currently set to \"" +
                                                ga.Build[i] + "\".", MessageType.Info);
                                        }
                                        ...
                                        if (GameAnalytics.SettingsGA.Platforms[i] == RuntimePlatform.IPhonePlayer)
                                        {
                                            ...
                                            ga.Build[i] = PlayerSettings.bundleVersion;
                                            EditorGUILayout.HelpBox(
                                                "Using iOS Player Settings Version* number as build number in events. \nBuild number is currently set to \"" +
                                                ga.Build[i] + "\".", MessageType.Info);
#else
                            ...
                            if (ga.SelectedPlatformGameID[i] >= 0)
                            {
                                ...
                                if (GUILayout.Button("Integration Status"))
                                {
                                    if (string.IsNullOrEmpty(ga.TokenGA))
                                        Application.OpenURL("https://go.gameanalytics.com/game/" +
                                                            ga.SelectedPlatformGameID[i] + "/initialize");
                                    else
                                        Application.OpenURL("https://go.gameanalytics.com/login?token=" + ga.TokenGA +
                                                            "&exp=" + ga.ExpireTime + "&goto=/game/" +
                                                            ga.SelectedPlatformGameID[i] + "/initialize");
                                }
                                ...
                                if (GUILayout.Button("Game Settings"))
                                {
                                    if (string.IsNullOrEmpty(ga.TokenGA))
                                        Application.OpenURL("https://go.gameanalytics.com/game/" +
                                                            ga.SelectedPlatformGameID[i] + "/settings");
                                    else
                                        Application.OpenURL("https://go.gameanalytics.com/login?token=" + ga.TokenGA +
                                                            "&exp=" + ga.ExpireTime + "&goto=/game/" +
                                                            ga.SelectedPlatformGameID[i] + "/settings");
                        ...
                        if (GUILayout.Button("Remove platform"))
                        {
                            ...
                            availablePlatforms = GameAnalytics.SettingsGA.GetAvailablePlatforms();
                    ...
                    selectedPlatformIndex =
                        EditorGUILayout.Popup("Platform to add", selectedPlatformIndex, availablePlatforms);
                    if (GUILayout.Button("Add platform"))
                    {
                        if (availablePlatforms[selectedPlatformIndex].Equals("WSA"))
                            GameAnalytics.SettingsGA.AddPlatform(RuntimePlatform.WSAPlayerARM);
                        else
                            GameAnalytics.SettingsGA.AddPlatform((RuntimePlatform) Enum.Parse(typeof(RuntimePlatform),
                                availablePlatforms[selectedPlatformIndex]));
                        availablePlatforms = GameAnalytics.SettingsGA.GetAvailablePlatforms();
                        selectedPlatformIndex = 0;
                    }
                    ...
                    if (!_customDimensionsIconOpen) GUI.color = new Color(0.54f, 0.54f, 0.54f);
                    if (GUILayout.Button(_customDimensionsIcon, GUIStyle.none, GUILayout.Width(12),
                            GUILayout.Height(12))) _customDimensionsIconOpen = !_customDimensionsIconOpen;
                    GUI.color = Color.white;
                    EditorGUIUtility.AddCursorRect(GUILayoutUtility.GetLastRect(), MouseCursor.Link);
                    GUILayout.FlexibleSpace();
                    ...
                    if (_customDimensionsIconOpen)
                    {
                        ...
                        if (GUI.Button(new Rect(tmpRect.x + 5, tmpRect.y + tmpRect.height - 25, 80, 20), "Learn more"))
                            Application.OpenURL(
                                "https://github.com/GameAnalytics/GA-SDK-UNITY/wiki/Settings#custom-dimensions");
                    }
                    ...
                    // Custom dimensions 1
                    ga.CustomDimensions01FoldOut = EditorGUILayout.Foldout(ga.CustomDimensions01FoldOut,
                        new GUIContent(
                            "   " + _customDimensions01.text + " (" + ga.CustomDimensions01.Count + " / " +
                            MaxNumberOfDimensions + " values)", _customDimensions01.tooltip));

                    if (ga.CustomDimensions01FoldOut)
                    {
                        var c1ToRemove = new List<int>();

                        for (var i = 0; i < ga.CustomDimensions01.Count; i++)
                        {
                            GUILayout.BeginHorizontal();
                            GUILayout.Label("", GUILayout.Width(21));
                            GUILayout.Label("-", GUILayout.Width(10));

                            ga.CustomDimensions01[i] =
                                ValidateCustomDimensionEditor(EditorGUILayout.TextField(ga.CustomDimensions01[i]));
                        ...
                        foreach (var i in c1ToRemove) ga.CustomDimensions01.RemoveAt(i);
                    ...
                    // Custom dimensions 2
                    ga.CustomDimensions02FoldOut = EditorGUILayout.Foldout(ga.CustomDimensions02FoldOut,
                        new GUIContent(
                            "   " + _customDimensions02.text + " (" + ga.CustomDimensions02.Count + " / " +
                            MaxNumberOfDimensions + " values)", _customDimensions02.tooltip));

                    if (ga.CustomDimensions02FoldOut)
                    {
                        var c2ToRemove = new List<int>();

                        for (var i = 0; i < ga.CustomDimensions02.Count; i++)
                        {
                            GUILayout.BeginHorizontal();
                            GUILayout.Label("", GUILayout.Width(21));
                            GUILayout.Label("-", GUILayout.Width(10));

                            ga.CustomDimensions02[i] =
                                ValidateCustomDimensionEditor(EditorGUILayout.TextField(ga.CustomDimensions02[i]));
                        ...
                        foreach (var i in c2ToRemove) ga.CustomDimensions02.RemoveAt(i);
                    ...
                    // Custom dimensions 3
                    ga.CustomDimensions03FoldOut = EditorGUILayout.Foldout(ga.CustomDimensions03FoldOut,
                        new GUIContent(
                            "   " + _customDimensions03.text + " (" + ga.CustomDimensions03.Count + " / " +
                            MaxNumberOfDimensions + " values)", _customDimensions03.tooltip));

                    if (ga.CustomDimensions03FoldOut)
                    {
                        var c3ToRemove = new List<int>();

                        for (var i = 0; i < ga.CustomDimensions03.Count; i++)
                        {
                            GUILayout.BeginHorizontal();
                            GUILayout.Label("", GUILayout.Width(21));
                            GUILayout.Label("-", GUILayout.Width(10));

                            ga.CustomDimensions03[i] =
                                ValidateCustomDimensionEditor(EditorGUILayout.TextField(ga.CustomDimensions03[i]));
                        ...
                        foreach (var i in c3ToRemove) ga.CustomDimensions03.RemoveAt(i);
                    ...
                    if (!_resourceTypesIconOpen) GUI.color = new Color(0.54f, 0.54f, 0.54f);
                    ...
                    if (_resourceTypesIconOpen)
                    {
                        ...
                        if (GUI.Button(new Rect(tmpRect.x + 5, tmpRect.y + tmpRect.height - 25, 80, 20), "Learn more"))
                            Application.OpenURL(
                                "https://github.com/GameAnalytics/GA-SDK-UNITY/wiki/Settings#resource-types");
                    }
                    ...
                    ga.ResourceCurrenciesFoldOut = EditorGUILayout.Foldout(ga.ResourceCurrenciesFoldOut,
                        new GUIContent(
                            "   " + _resourceCurrrencies.text + " (" + ga.ResourceCurrencies.Count + " / " +
                            MaxNumberOfDimensions + " values)", _resourceCurrrencies.tooltip));

                    if (ga.ResourceCurrenciesFoldOut)
                    {
                        var rcToRemove = new List<int>();

                        for (var i = 0; i < ga.ResourceCurrencies.Count; i++)
                        {
                            GUILayout.BeginHorizontal();
                            GUILayout.Label("", GUILayout.Width(21));
                            GUILayout.Label("-", GUILayout.Width(10));
                            ga.ResourceCurrencies[i] =
                                ValidateResourceCurrencyEditor(EditorGUILayout.TextField(ga.ResourceCurrencies[i]));
                        ...
                        foreach (var i in rcToRemove) ga.ResourceCurrencies.RemoveAt(i);
                    ...
                    ga.ResourceItemTypesFoldOut = EditorGUILayout.Foldout(ga.ResourceItemTypesFoldOut,
                        new GUIContent(
                            "   " + _resourceItemTypes.text + " (" + ga.ResourceItemTypes.Count + " / " +
                            MaxNumberOfDimensions + " values)", _resourceItemTypes.tooltip));

                    if (ga.ResourceItemTypesFoldOut)
                    {
                        var ritToRemove = new List<int>();

                        for (var i = 0; i < ga.ResourceItemTypes.Count; i++)
                        {
                            GUILayout.BeginHorizontal();
                            GUILayout.Label("", GUILayout.Width(21));
                            GUILayout.Label("-", GUILayout.Width(10));
                            //string tmp = ga.ResourceTypes[i];
                            ga.ResourceItemTypes[i] =
                                ValidateResourceItemTypeEditor(EditorGUILayout.TextField(ga.ResourceItemTypes[i]));
                        ...
                        foreach (var i in ritToRemove) ga.ResourceItemTypes.RemoveAt(i);
                ...
                else if (ga.CurrentInspectorState == Settings.InspectorStates.Pref)
                {
                    ...
                    if (!_advancedSettingsIconOpen) GUI.color = new Color(0.54f, 0.54f, 0.54f);
                    ...
                    GUILayout.BeginHorizontal();
                    GUILayout.Label("", GUILayout.Width(-18));
                    ga.UseManualSessionHandling =
                        EditorGUILayout.Toggle("", ga.UseManualSessionHandling, GUILayout.Width(35));
                    ...
                    GUILayout.BeginHorizontal();
                    GUILayout.Label("", GUILayout.Width(-18));
                    ga.UsePlayerSettingsBuildNumber =
                        EditorGUILayout.Toggle("", ga.UsePlayerSettingsBuildNumber, GUILayout.Width(35));
...
#if UNITY_5_6_OR_NEWER
                        EditorGUILayout.HelpBox(
                            "PLEASE NOTICE: The SDK will use the Version* number (Android, iOS) from Player Settings as the build number in events.",
                            MessageType.Info);
#else
                    ...
                    var tmpFpsCriticalThreshold = 0;
                    if (int.TryParse(EditorGUILayout.TextField(ga.FpsCriticalThreshold.ToString(), GUILayout.Width(45)),
                            out tmpFpsCriticalThreshold))
                    ...
                    if (!_debugSettingsIconOpen) GUI.color = new Color(0.54f, 0.54f, 0.54f);
                    ...
                    if (_debugSettingsIconOpen)
                    {
                        ...
                        if (GUI.Button(new Rect(tmpRect.x + 5, tmpRect.y + tmpRect.height - 25, 80, 20), "Learn more"))
                            Application.OpenURL(
                                "https://github.com/GameAnalytics/GA-SDK-UNITY/wiki/Settings#debug-settings");
                    }
            ...
            if (GUI.changed) EditorUtility.SetDirty(ga);
        }
        ...
        public static void SignupUser(Settings ga, GA_SignUp signup)
        {
            ...
            var data = System.Text.Encoding.UTF8.GetBytes(GA_MiniJSON.Serialize(jsonTable));

#if UNITY_2017_1_OR_NEWER
            var www = new UnityWebRequest(_gaUrl + "user", UnityWebRequest.kHttpVerbPOST);
            ...
            var headers = GA_EditorUtilities.WWWHeaders();
            foreach (var entry in headers) www.SetRequestHeader(entry.Key, entry.Value);
#else
...
#endif
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(text))
                {
                    ...
                    {
                        var errorList = returnParam["errors"] as IList<object>;
                        if (errorList != null && errorList.Count > 0)
                        {
                            var errors = errorList[0] as IDictionary<string, object>;
                            if (errors.ContainsKey("msg")) error = errors["msg"].ToString();
                        }
                    }
                }
...
#endif
                {
                    ...
                    {
                        var resultList = returnParam["results"] as IList<object>;
                        var results = resultList[0] as IDictionary<string, object>;
                ...
                {
                    Debug.LogError(
                        "Failed to sign up. GameAnalytics request not successful. API was changed. Please update your SDK to the latest version: " +
                        www.error + " " + error);
                    SetLoginStatus(
                        "Failed to sign up. GameAnalytics request not successful. API was changed. Please update your SDK to the latest version.",
                        ga);
...
#if UNITY_2017_1_OR_NEWER
            var www = new UnityWebRequest(_gaUrl + "token", UnityWebRequest.kHttpVerbPOST);
            var uH = new UploadHandlerRaw(data)
            {
                contentType = "application/json"
            };
...
#endif
        {
            ...
            try
            {
                ...
                {
                    Debug.LogError(
                        "Failed to login. GameAnalytics request not successful. API was changed. Please update your SDK to the latest version: " +
                        www.error + " " + error);
                    SetLoginStatus(
                        "Failed to login. GameAnalytics request not successful. API was changed. Please update your SDK to the latest version.",
                        ga);
        ...
        {
#if UNITY_2017_1_OR_NEWER
            var www = UnityWebRequest.Get(_gaUrl + "user");
            var headers = GA_EditorUtilities.WWWHeadersWithAuthorization(ga.TokenGA);
            foreach (var entry in headers) www.SetRequestHeader(entry.Key, entry.Value);
#else
            ...
            try
            {
...
#endif
                {
                    ...
                    else if (returnParam != null)
                    {
                        var resultList = returnParam["results"] as IList<object>;
                        var results = resultList[0] as IDictionary<string, object>;
                        var orgs = results["organizations"] as IDictionary<string, object>;
                        var studioList = results["studios"] as IList<object>;

                        var organizationMap = new Dictionary<string, Organization>();
                        var returnOrganizations = new List<Organization>();
                        foreach (var pair in orgs)
                        {
                            var organization = pair.Value as IDictionary<string, object>;
                            var o = new Organization(organization["name"].ToString(), organization["id"].ToString());
                            returnOrganizations.Add(o);
                            organizationMap.Add(o.ID, o);
                        }

                        for (var s = 0; s < studioList.Count; s++)
                        {
                            var studio = studioList[s] as IDictionary<string, object>;

                            if ((!studio.ContainsKey("demo") || !(bool) studio["demo"]) &&
                                (!studio.ContainsKey("archived") || !(bool) studio["archived"]))
                            {
                                var returnGames = new List<Game>();

                                var gamesList = (List<object>) studio["games"];
                                for (var g = 0; g < gamesList.Count; g++)
                                {
                                    var game = gamesList[g] as IDictionary<string, object>;

                                    if ((!game.ContainsKey("archived") || !(bool) game["archived"]) &&
                                        (!game.ContainsKey("disabled") || !(bool) game["disabled"]))
                                        returnGames.Add(new Game(game["name"].ToString(),
                                            int.Parse(game["id"].ToString()), game["key"].ToString(),
                                            game["secret"].ToString()));
                                }

                                var st = new Studio(studio["name"].ToString(), studio["id"].ToString(),
                                    studio["org_id"].ToString(), returnGames);
                        ...
                        {
                            var autoSelectedPlatform = false;
                            for (var i = 0; i < GameAnalytics.SettingsGA.Platforms.Count; ++i)
                            {
                                var platform = GameAnalytics.SettingsGA.Platforms[i];
                            ...
                            SetLoginStatus(
                                autoSelectedPlatform
                                    ? "Received data. Autoselected platform.."
                                    : "Received data. Add a platform..", ga);
                        }
                        else
                        {
                ...
                {
                    Debug.LogError(
                        "Failed to get data. GameAnalytics request not successful. API was changed. Please update your SDK to the latest version: " +
                        www.error + " " + error);
                    SetLoginStatus(
                        "Failed to get data. GameAnalytics request not successful. API was changed. Please update your SDK to the latest version.",
                        ga);
        ...
        public static void CreateGame(Settings ga, GA_SignUp signup, int organizationIndex, int studioIndex,
            string gameTitle, string googlePlayPublicKey, RuntimePlatform platform, AppFiguresGame appFiguresGame)
        {
            var jsonTable = new Hashtable();
            ...
            var url = _gaUrl + "studios/" + ga.Organizations[organizationIndex].Studios[studioIndex].ID + "/games";
#if UNITY_2017_1_OR_NEWER
...
#endif
            GA_ContinuationManager.StartCoroutine(CreateGameFrontend(www, ga, signup, platform, appFiguresGame),
                () => www.isDone);
        }

#if UNITY_2017_1_OR_NEWER
        private static IEnumerator CreateGameFrontend(UnityWebRequest www, Settings ga, GA_SignUp signup,
            RuntimePlatform platform, AppFiguresGame appFiguresGame)
#else
...
#endif
        {
            ...
            try
            {
                ...
                {
                    Debug.LogError(
                        "Failed to create game. GameAnalytics request not successful. API was changed. Please update your SDK to the latest version: " +
                        www.error + " " + error);
                    SetLoginStatus(
                        "Failed to create game. GameAnalytics request not successful. API was changed. Please update your SDK to the latest version.",
                        ga);
        ...
        {
#if UNITY_2017_1_OR_NEWER
            var www = UnityWebRequest.Get(_gaUrl + "apps/search?query=" + UnityWebRequest.EscapeURL(ga.GameName));
            var headers = GA_EditorUtilities.WWWHeadersWithAuthorization(ga.TokenGA);
            foreach (var pair in headers) www.SetRequestHeader(pair.Key, pair.Value);
            GA_ContinuationManager.StartCoroutine(GetAppFiguresFrontend(www, ga, signup, ga.GameName),
                () => www.isDone);
#else
            WWW www =
 new WWW(_gaUrl + "apps/search?query=" + WWW.EscapeURL(ga.GameName), null, GA_EditorUtilities.WWWHeadersWithAuthorization(ga.TokenGA));
            ...
            if (ga.AmazonIcon == null)
            {
                ...
                var wwwAmazon =
                    UnityWebRequestTexture.GetTexture(
                        "http://public.gameanalytics.com/resources/images/sdk_doc/appstore_icons/amazon.png");
                GA_ContinuationManager.StartCoroutine(
                    signup.GetAppStoreIconTexture(wwwAmazon, "amazon_appstore", signup), () => wwwAmazon.isDone);
#else
                WWW wwwAmazon =
 new WWW("http://public.gameanalytics.com/resources/images/sdk_doc/appstore_icons/amazon.png");
            ...
            {
#if UNITY_2017_1_OR_NEWER
                var wwwGoogle = UnityWebRequestTexture.GetTexture(
                    "http://public.gameanalytics.com/resources/images/sdk_doc/appstore_icons/google_play.png");
                GA_ContinuationManager.StartCoroutine(signup.GetAppStoreIconTexture(wwwGoogle, "google_play", signup),
                    () => wwwGoogle.isDone);
#else
                WWW wwwGoogle =
 new WWW("http://public.gameanalytics.com/resources/images/sdk_doc/appstore_icons/google_play.png");
            ...
            if (ga.iosIcon == null)
            {
                ...
                var wwwIos =
                    UnityWebRequestTexture.GetTexture(
                        "http://public.gameanalytics.com/resources/images/sdk_doc/appstore_icons/ios.png");
                GA_ContinuationManager.StartCoroutine(signup.GetAppStoreIconTexture(wwwIos, "apple:ios", signup),
                    () => wwwIos.isDone);
#else
            ...
            if (ga.macIcon == null)
            {
                ...
                var wwwMac =
                    UnityWebRequestTexture.GetTexture(
                        "http://public.gameanalytics.com/resources/images/sdk_doc/appstore_icons/mac.png");
                GA_ContinuationManager.StartCoroutine(signup.GetAppStoreIconTexture(wwwMac, "apple:mac", signup),
                    () => wwwMac.isDone);
#else
            ...
            {
#if UNITY_2017_1_OR_NEWER
                var wwwWindowsPhone = UnityWebRequestTexture.GetTexture(
                    "http://public.gameanalytics.com/resources/images/sdk_doc/appstore_icons/windows_phone.png");
                GA_ContinuationManager.StartCoroutine(
                    signup.GetAppStoreIconTexture(wwwWindowsPhone, "windows_phone", signup),
                    () => wwwWindowsPhone.isDone);
#else
                WWW wwwWindowsPhone =
 new WWW("http://public.gameanalytics.com/resources/images/sdk_doc/appstore_icons/windows_phone.png");
...
#if UNITY_2017_1_OR_NEWER
        private static IEnumerator GetAppFiguresFrontend(UnityWebRequest www, Settings ga, GA_SignUp signup,
            string gameName)
#else
            ...
            try
            {
...
#endif
                {
                    ...
                    else if (returnParam != null)
                    {
                        ...
                        var appFiguresGames = new List<AppFiguresGame>();
                        for (var s = 0; s < resultList.Count; s++)
                        {
                            var result = resultList[s] as IDictionary<string, object>;

                            var name = result["title"].ToString();
                            var appID = result["store_id"].ToString();
                            var store = result["store"].ToString();
                            var developer = result["developer"].ToString();
                            var iconUrl = result["image"].ToString();
                ...
                else
                {
...
#endif
                    {
                        ...
                        {
                            AssetDatabase.LoadAssetAtPath("Assets/Resources/GameAnalytics/Settings.asset",
                                typeof(Settings))
                        };
                        ga.CurrentInspectorState = Settings.InspectorStates.Account;
                        var message = "Please sign-in and try again to search for your game in the stores.";
                    ...
                    {
                        Debug.LogError(
                            "Failed to find app. GameAnalytics request not successful. API was changed. Please update your SDK to the latest version: " +
                            www.error + " " + error);
                        SetLoginStatus(
                            "Failed to find app. GameAnalytics request not successful. API was changed. Please update your SDK to the latest version.",
                            ga);
        ...
        private static void SelectStudio(int index, Settings ga, int platform)
        {
            ...
            {
                if (ga.IsGameKeyValid(platform,
                        ga.Organizations[ga.SelectedOrganization[platform] - 1].Studios[ga.SelectedStudio[platform] - 1]
                            .Games[0].GameKey) &&
                    ga.IsSecretKeyValid(platform,
                        ga.Organizations[ga.SelectedOrganization[platform] - 1].Studios[ga.SelectedStudio[platform] - 1]
                            .Games[0].SecretKey))
        ...
        private static void SelectGame(int index, Settings ga, int platform)
        {
            ...
            else if (ga.IsGameKeyValid(platform,
                         ga.Organizations[ga.SelectedOrganization[platform] - 1]
                             .Studios[ga.SelectedStudio[platform] - 1].Games[index - 1].GameKey) &&
                     ga.IsSecretKeyValid(platform,
                         ga.Organizations[ga.SelectedOrganization[platform] - 1]
                             .Studios[ga.SelectedStudio[platform] - 1].Games[index - 1].SecretKey))
            {
                ga.SelectedPlatformOrganization[platform] =
                    ga.Organizations[ga.SelectedOrganization[platform] - 1].Name;
                ga.SelectedPlatformStudio[platform] = ga.Organizations[ga.SelectedOrganization[platform] - 1]
                    .Studios[ga.SelectedStudio[platform] - 1].Name;
                ga.SelectedPlatformGame[platform] = ga.Organizations[ga.SelectedOrganization[platform] - 1]
                    .Studios[ga.SelectedStudio[platform] - 1].Games[index - 1].Name;
                ga.SelectedPlatformGameID[platform] = ga.Organizations[ga.SelectedOrganization[platform] - 1]
                    .Studios[ga.SelectedStudio[platform] - 1].Games[index - 1].ID;
                ga.UpdateGameKey(platform,
                    ga.Organizations[ga.SelectedOrganization[platform] - 1].Studios[ga.SelectedStudio[platform] - 1]
                        .Games[index - 1].GameKey);
                ga.UpdateSecretKey(platform,
                    ga.Organizations[ga.SelectedOrganization[platform] - 1].Studios[ga.SelectedStudio[platform] - 1]
                        .Games[index - 1].SecretKey);
            ...
            else
            {
                ...
                {
                    Debug.LogError(
                        "[GameAnalytics] Game key already exists for another platform. Platforms can't use the same key.");
                    ga.SelectedGame[platform] = 0;
                }
                else if (!ga.IsSecretKeyValid(platform,
                             ga.Organizations[ga.SelectedOrganization[platform] - 1]
                                 .Studios[ga.SelectedStudio[platform] - 1].Games[index - 1].SecretKey))
                {
                    Debug.LogError(
                        "[GameAnalytics] Secret key already exists for another platform. Platforms can't use the same key.");
                    ga.SelectedGame[platform] = 0;
                }
            }
        }
        ...
        {
            if (Settings.CheckingForUpdates) return;

            Settings.CheckingForUpdates = true;
#if UNITY_2017_1_OR_NEWER
            var www = UnityWebRequest.Get("https://s3.amazonaws.com/public.gameanalytics.com/sdk_status/current.json");
#else
        ...
        private static void GetChangeLogsAndShowUpdateWindow(string newVersion)
        {
            ...
            var www = UnityWebRequest.Get(
                "https://s3.amazonaws.com/public.gameanalytics.com/sdk_status/change_logs.json");
#else
            WWW www = new WWW("https://s3.amazonaws.com/public.gameanalytics.com/sdk_status/change_logs.json");
#endif
            GA_ContinuationManager.StartCoroutine(GetChangeLogsAndShowUpdateWindowCoroutine(www, newVersion),
                () => www.isDone);
        }
...
#endif
        {
            ...
            try
            {
...
#endif
                {
                    ...
                    text = www.text;
#endif
                    var returnParam = GA_MiniJSON.Deserialize(text) as IDictionary<string, object>;
                    if (returnParam.ContainsKey("unity"))
                    {
                        var unityParam = returnParam["unity"] as IDictionary<string, object>;
                        if (unityParam.ContainsKey("version"))
                        {
                            var newVersion = (returnParam["unity"] as IDictionary<string, object>)["version"]
                                .ToString();
...
#endif
        {
            ...
            try
            {
...
#endif
                {
                    ...
                    var unity = returnParam["unity"] as IList<object>;
                    var newChanges = "";
                    for (var i = 0; i < unity.Count; i++)
                    {
                        var unityHash = unity[i] as IDictionary<string, object>;
                        var changes = unityHash["changes"] as IList<object>;

                        if (unityHash["version"].ToString() == Settings.VERSION) break;
                        ...
                        for (var u = 0; u < changes.Count; u++)
                            if (string.IsNullOrEmpty(newChanges))
                                newChanges = "- " + changes[u].ToString();
                            else
                                newChanges += "\n- " + changes[u].ToString();

                        if (unityHash["version"].ToString() == newVersion) GA_UpdateWindow.SetNewVersion(newVersion);
                    }

                    var skippedVersion = EditorPrefs.GetString("ga_skip_version" + "-" + Application.dataPath, "");

                    GA_UpdateWindow.SetChanges(newChanges);
                    if (!skippedVersion.Equals(newVersion)) OpenUpdateWindow();
        ...
        private static void OpenUpdateWindow()
        {
...
#endif
            {
                ...
                var updateWindow = (GA_UpdateWindow) EditorWindow.GetWindow(typeof(GA_UpdateWindow), true);
        ...
        private static string PlatformToString(RuntimePlatform platform)
        {
            ...
            if (platform == RuntimePlatform.IPhonePlayer) result = "iOS";
        ...
        private static bool IsNewVersion(string newVersion, string currentVersion)
        {
            var newVersionInts = GetVersionIntegersFromString(newVersion);
            var currentVersionInts = GetVersionIntegersFromString(currentVersion);

            if (newVersionInts == null || currentVersionInts == null) return false;

            // compare majorVersion
            if (newVersionInts[0] > currentVersionInts[0])
                return true;
            else if (newVersionInts[0] < currentVersionInts[0]) return false;

            // compare minorVersion (majorVersion is unchanged)
            if (newVersionInts[1] > currentVersionInts[1])
                return true;
            else if (newVersionInts[1] < currentVersionInts[1]) return false;

            // compare patchnumber (majorVersion, minorVersion is unchanged)
            if (newVersionInts[2] > currentVersionInts[2]) return true;
        ...
        private static int[] GetVersionIntegersFromString(string versionString)
        {
            var versionNumbers = versionString.Split('.');
            if (versionNumbers.Length != 3) return null;

            // container for validated version integers
            var validatedVersionNumbers = new int[3];

            // verify int parsing
            var isIntMajorVersion = int.TryParse(versionNumbers[0], out validatedVersionNumbers[0]);
            var isIntMinorVersion = int.TryParse(versionNumbers[1], out validatedVersionNumbers[1]);
            var isIntPatchnumber = int.TryParse(versionNumbers[2], out validatedVersionNumbers[2]);
        ...
        private static void DrawLinkButton(GUIContent content, GUIStyle style, string url,
        ...
        private static void DrawButton(GUIContent content, GUIStyle style, Action action,
            params GUILayoutOption[] options)
        {
            if (GUILayout.Button(content, style, options)) action();
            EditorGUIUtility.AddCursorRect(GUILayoutUtility.GetLastRect(), MouseCursor.Link);
        }

        private static void DrawButtonWithFlexibleSpace(string text, GUIStyle style, Action action,
            params GUILayoutOption[] options)
        {
            GUILayout.BeginHorizontal();
            GUILayout.FlexibleSpace();
        ...
        private string ValidateResourceCurrencyEditor(string currency)
        {
            if (!StringMatch(currency, "^[A-Za-z]+$"))
            {
                if (currency != null)
                    Debug.LogError(
                        "Validation fail - resource currency: Cannot contain other characters than 'A-Za-z'. String:'" +
                        currency + "'");
                return "Empty";
            }

            if (ConsistsOfWhiteSpace(currency)) return "Empty";
            return currency;
        }
        ...
        private string ValidateResourceItemTypeEditor(string itemType)
        {
            ...
            if (!StringMatch(itemType, "^[A-Za-z0-9\\s\\-_\\.\\(\\)\\!\\?]{1,64}$"))
            {
                if (itemType != null)
                    Debug.LogError(
                        "Validation fail - resource itemType: Cannot contain other characters than A-z, 0-9, -_., ()!?. String: '" +
                        itemType + "'");
                return "Empty";
            }

            if (ConsistsOfWhiteSpace(itemType)) return "Empty";
            return itemType;
        }
        ...
        private string ValidateCustomDimensionEditor(string customDimension)
        {
            ...
            if (!StringMatch(customDimension, "^[A-Za-z0-9\\s\\-_\\.\\(\\)\\!\\?]{1,32}$"))
            {
                if (customDimension != null)
                    Debug.LogError(
                        "Validation fail - custom dimension: Cannot contain other characters than A-z, 0-9, -_., ()!?. String: '" +
                        customDimension + "'");
                return "Empty";
            }

            if (ConsistsOfWhiteSpace(customDimension)) return "Empty";
