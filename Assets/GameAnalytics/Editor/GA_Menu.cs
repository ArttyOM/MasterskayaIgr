...
namespace GameAnalyticsSDK.Editor
{
    public static class GA_Menu
    {
        ...
        private static void SelectGASettings()
        {
            Selection.activeObject = GameAnalytics.SettingsGA;
        }

        [MenuItem("Window/GameAnalytics/Setup Guide", false, 100)]
        private static void SetupAndTour()
        {
            var signup = ScriptableObject.CreateInstance<GA_SignUp>();
        ...
        private static void AddGASystemTracker()
        {
            if (Object.FindObjectOfType(typeof(GameAnalytics)) == null)
            {
                var go = PrefabUtility.InstantiatePrefab(
                    AssetDatabase.LoadAssetAtPath(GameAnalytics.WhereIs("GameAnalytics.prefab", "Prefab"),
            ...
            {
                Debug.LogWarning(
                    "A GameAnalytics object already exists in this scene - you should never have more than one per scene!");
            }
        }

        [MenuItem("Window/GameAnalytics/PlayMaker/Toggle Scripts", false, 400)]
        private static void TogglePlayMaker()
        {
            var enabled = false;
            var fail = false;

            var searchText = "#if false";
            var replaceText = "#if true";

            var _files = new string[]
            {
                "GAInitialize.cs",
                "GetABTestingId.cs",
