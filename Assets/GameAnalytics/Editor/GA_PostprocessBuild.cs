...
#endif
    {
        ...
        private static void UpdateMoPub()
        {
            ...
            if (TypeExists(mopubTypes))
                UpdateDefines(gameanalytics_mopub, true,
                    new BuildTargetGroup[] {BuildTargetGroup.iOS, BuildTargetGroup.Android});
            else
                UpdateDefines(gameanalytics_mopub, false,
                    new BuildTargetGroup[] {BuildTargetGroup.iOS, BuildTargetGroup.Android});
        }
        ...
        private static void UpdateFyber()
        {
            ...
            if (TypeExists(fyberTypes))
                UpdateDefines(gameanalytics_fyber, true,
                    new BuildTargetGroup[] {BuildTargetGroup.iOS, BuildTargetGroup.Android});
            else
                UpdateDefines(gameanalytics_fyber, false,
                    new BuildTargetGroup[] {BuildTargetGroup.iOS, BuildTargetGroup.Android});
        }
        ...
        private static void UpdateIronSource()
        {
            ...
            if (TypeExists(ironSourceTypes))
                UpdateDefines(gameanalytics_ironsource, true,
                    new BuildTargetGroup[] {BuildTargetGroup.iOS, BuildTargetGroup.Android});
            else
                UpdateDefines(gameanalytics_ironsource, false,
                    new BuildTargetGroup[] {BuildTargetGroup.iOS, BuildTargetGroup.Android});
        }
        ...
        {
            var topOnTypes = new string[]
            {
                "AnyThinkAds.Api.ATBannerAd", "AnyThinkAds.Api.ATInterstitialAd", "AnyThinkAds.Api.ATRewardedVideo",
                "AnyThinkAds.Api.ATNativeAd"
            };
            if (TypeExists(topOnTypes))
                UpdateDefines(gameanalytics_topon, true,
                    new BuildTargetGroup[] {BuildTargetGroup.iOS, BuildTargetGroup.Android});
            else
                UpdateDefines(gameanalytics_topon, false,
                    new BuildTargetGroup[] {BuildTargetGroup.iOS, BuildTargetGroup.Android});
        }
        ...
        private static void UpdateMax()
        {
            ...
            if (TypeExists(maxTypes))
                UpdateDefines(gameanalytics_max, true,
                    new BuildTargetGroup[] {BuildTargetGroup.iOS, BuildTargetGroup.Android});
            else
                UpdateDefines(gameanalytics_max, false,
                    new BuildTargetGroup[] {BuildTargetGroup.iOS, BuildTargetGroup.Android});
        }
        ...
        {
            var aequusTypes = new string[]
                {"Mobi.Aequus.Sdk.Aequus", "Mobi.Aequus.Sdk.AequusILRDListener", "Mobi.Aequus.Sdk.ImpressionData"};
            if (TypeExists(aequusTypes))
                UpdateDefines(gameanalytics_aequus, true,
                    new BuildTargetGroup[] {BuildTargetGroup.iOS, BuildTargetGroup.Android});
            else
                UpdateDefines(gameanalytics_aequus, false,
                    new BuildTargetGroup[] {BuildTargetGroup.iOS, BuildTargetGroup.Android});
        }
        ...
        private static void UpdateHyperBid()
        {
            var topOnTypes = new string[]
            {
                "HyperBid.Api.HBBannerAd", "HyperBid.Api.HBInterstitialAd", "HyperBid.Api.HBRewardedVideo",
                "HyperBid.Api.HBNativeAd"
            };
            if (TypeExists(topOnTypes))
                UpdateDefines(gameanalytics_hyperbid, true,
                    new BuildTargetGroup[] {BuildTargetGroup.iOS, BuildTargetGroup.Android});
            else
                UpdateDefines(gameanalytics_hyperbid, false,
                    new BuildTargetGroup[] {BuildTargetGroup.iOS, BuildTargetGroup.Android});
        }
        ...
        private static void UpdateAdMob()
        {
            var topOnTypes = new string[]
            {
                "GoogleMobileAds.Api.AdRequest", "GoogleMobileAds.Api.BannerView", "GoogleMobileAds.Api.InterstitialAd",
                "GoogleMobileAds.Api.RewardedAd", "GoogleMobileAds.Api.RewardedInterstitialAd"
            };
            if (TypeExists(topOnTypes))
                UpdateDefines(gameanalytics_admob, true,
                    new BuildTargetGroup[] {BuildTargetGroup.iOS, BuildTargetGroup.Android});
            else
                UpdateDefines(gameanalytics_admob, false,
                    new BuildTargetGroup[] {BuildTargetGroup.iOS, BuildTargetGroup.Android});
        }
        ...
        public static void OnPostprocessBuild(BuildTarget buildTarget, string path)
        {
            if (buildTarget == BuildTarget.iOS || buildTarget == BuildTarget.tvOS)
            {
                ...
                hyperbidContents =
 hyperbidContents.Replace("#if gameanalytics_hyperbid_enabled", "").Replace("#endif", "");
