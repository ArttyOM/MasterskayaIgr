...
public class GAMaxIntegration
{
 ...
 (adUnitId, adInfo) => runCallback("INTER", adInfo, callback);
 ...
 (adUnitId, adInfo) => runCallback("REWARDED", adInfo, callback);
        MaxSdkCallbacks.CrossPromo.OnAdRevenuePaidEvent +=
 (adUnitId, adInfo) => runCallback("XPROMO", adInfo, callback);
 ...
 (adUnitId, adInfo) => runCallback("REWARDED_INTER", adInfo, callback);
        _subscribed = true;
#endif
    }
}