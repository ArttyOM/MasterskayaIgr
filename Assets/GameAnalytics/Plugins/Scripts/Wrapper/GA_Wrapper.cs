...
namespace GameAnalyticsSDK.Wrapper
{
    public partial class GA_Wrapper
    {
        ...
        {
            if (GameAnalytics.SettingsGA.InfoLogEditor) Debug.Log("setAvailableCustomDimensions01(" + list + ")");
        }

        private static void configureAvailableCustomDimensions02(string list)
        {
            if (GameAnalytics.SettingsGA.InfoLogEditor) Debug.Log("setAvailableCustomDimensions02(" + list + ")");
        }

        private static void configureAvailableCustomDimensions03(string list)
        {
            if (GameAnalytics.SettingsGA.InfoLogEditor) Debug.Log("setAvailableCustomDimensions03(" + list + ")");
        }

        private static void configureAvailableResourceCurrencies(string list)
        {
            if (GameAnalytics.SettingsGA.InfoLogEditor) Debug.Log("setAvailableResourceCurrencies(" + list + ")");
        }

        private static void configureAvailableResourceItemTypes(string list)
        {
            if (GameAnalytics.SettingsGA.InfoLogEditor) Debug.Log("setAvailableResourceItemTypes(" + list + ")");
        }

        private static void configureSdkGameEngineVersion(string unitySdkVersion)
        {
            if (GameAnalytics.SettingsGA.InfoLogEditor) Debug.Log("GameAnalytics SDK version: " + unitySdkVersion);
        }
        ...
        {
            if (GameAnalytics.SettingsGA.InfoLogEditor) Debug.Log("setBuild(" + build + ")");
        }

        private static void configureUserId(string userId)
        {
            if (GameAnalytics.SettingsGA.InfoLogEditor) Debug.Log("configureUserId(" + userId + ")");
        }

        private static void configureAutoDetectAppVersion(bool flag)
        {
            if (GameAnalytics.SettingsGA.InfoLogEditor) Debug.Log("configureAutoDetectAppVersion(" + flag + ")");
        }

        public static string getUserId()
        {
            if (GameAnalytics.SettingsGA.InfoLogEditor) Debug.Log("getUserId");
        ...
        {
            if (GameAnalytics.SettingsGA.InfoLogEditor) Debug.Log("initialize(" + gamekey + "," + gamesecret + ")");
        }

        private static void setCustomDimension01(string customDimension)
        {
            if (GameAnalytics.SettingsGA.InfoLogEditor) Debug.Log("setCustomDimension01(" + customDimension + ")");
        }

        private static void setCustomDimension02(string customDimension)
        {
            if (GameAnalytics.SettingsGA.InfoLogEditor) Debug.Log("setCustomDimension02(" + customDimension + ")");
        }

        private static void setCustomDimension03(string customDimension)
        {
            if (GameAnalytics.SettingsGA.InfoLogEditor) Debug.Log("setCustomDimension03(" + customDimension + ")");
        }

        private static void setGlobalCustomEventFields(string customFields)
        {
            if (GameAnalytics.SettingsGA.InfoLogEditor) Debug.Log("setGlobalCustomEventFields(" + customFields + ")");
        }
...
#if !UNITY_IOS && !UNITY_TVOS
        private static void addBusinessEvent(string currency, int amount, string itemType, string itemId,
            string cartType, string fields, bool mergeFields)
        {
            if (GameAnalytics.SettingsGA.InfoLogEditor)
                Debug.Log("addBusinessEvent(" + currency + "," + amount + "," + itemType + "," + itemId + "," +
                          cartType + ")");
        }
#endif

        private static void addResourceEvent(int flowType, string currency, float amount, string itemType,
            string itemId, string fields, bool mergeFields)
        {
            if (GameAnalytics.SettingsGA.InfoLogEditor)
                Debug.Log("addResourceEvent(" + flowType + "," + currency + "," + amount + "," + itemType + "," +
                          itemId + ")");
        }

        private static void addProgressionEvent(int progressionStatus, string progression01, string progression02,
            string progression03, string fields, bool mergeFields)
        {
            if (GameAnalytics.SettingsGA.InfoLogEditor)
                Debug.Log("addProgressionEvent(" + progressionStatus + "," + progression01 + "," + progression02 + "," +
                          progression03 + ")");
        }

        private static void addProgressionEventWithScore(int progressionStatus, string progression01,
            string progression02, string progression03, int score, string fields, bool mergeFields)
        {
            if (GameAnalytics.SettingsGA.InfoLogEditor)
                Debug.Log("addProgressionEvent(" + progressionStatus + "," + progression01 + "," + progression02 + "," +
                          progression03 + "," + score + ")");
        ...
        {
            if (GameAnalytics.SettingsGA.InfoLogEditor) Debug.Log("addDesignEvent(" + eventId + ")");
        }
        ...
        {
            if (GameAnalytics.SettingsGA.InfoLogEditor) Debug.Log("addErrorEvent(" + severity + "," + message + ")");
        }

        private static void addAdEventWithDuration(int adAction, int adType, string adSdkName, string adPlacement,
            long duration, string fields, bool mergeFields)
        {
            if (GameAnalytics.SettingsGA.InfoLogEditor)
        ...
        private static void addAdEventWithReason(int adAction, int adType, string adSdkName, string adPlacement,
            int noAdReason, string fields, bool mergeFields)
        {
            if (GameAnalytics.SettingsGA.InfoLogEditor)
                Debug.Log("addAdEvent(" + adAction + "," + adType + "," + adSdkName + "," + adPlacement + "," +
                          noAdReason + ")");
        ...
        {
            if (GameAnalytics.SettingsGA.InfoLogEditor) Debug.Log("GameAnalytics setVerboseLog(" + enabled + ")");
        }

        private static void setManualSessionHandling(bool enabled)
        {
            if (GameAnalytics.SettingsGA.InfoLogEditor) Debug.Log("setManualSessionHandling(" + enabled + ")");
        }

        private static void setEventSubmission(bool enabled)
        {
            if (GameAnalytics.SettingsGA.InfoLogEditor) Debug.Log("setEventSubmission(" + enabled + ")");
        }
        ...
        {
            if (GameAnalytics.SettingsGA.InfoLogEditor) Debug.Log("gameAnalyticsStartSession()");
        }

        private static void gameAnalyticsEndSession()
        {
            if (GameAnalytics.SettingsGA.InfoLogEditor) Debug.Log("gameAnalyticsEndSession()");
        }

        // ----------------------- REMOTE CONFIGS ---------------------- //
        private static string getRemoteConfigsValueAsString(string key, string defaultValue)
        {
            if (GameAnalytics.SettingsGA.InfoLogEditor) Debug.Log("getRemoteConfigsValueAsString()");
            return defaultValue;
        }

        private static bool isRemoteConfigsReady()
        {
            if (GameAnalytics.SettingsGA.InfoLogEditor) Debug.Log("isRemoteConfigsReady()");
            return false;
        }

        private static string getRemoteConfigsContentAsString()
        {
            if (GameAnalytics.SettingsGA.InfoLogEditor) Debug.Log("getRemoteConfigsContentAsString()");
            return "";
        }

        private static string getABTestingId()
        {
            if (GameAnalytics.SettingsGA.InfoLogEditor) Debug.Log("getABTestingId()");
            return "";
        }

        private static string getABTestingVariantId()
        {
            if (GameAnalytics.SettingsGA.InfoLogEditor) Debug.Log("getABTestingVariantId()");
            return "";
        }
        ...
        {
#if UNITY_EDITOR
            if (GAValidator.ValidateBuild(build)) configureBuild(build);
#else
        ...
        {
#if UNITY_EDITOR
            if (GAValidator.ValidateUserId(userId)) configureUserId(userId);
#else
        ...
        {
#if UNITY_EDITOR
            if (GAValidator.ValidateKeys(gamekey, gamesecret)) initialize(gamekey, gamesecret);
#else
        ...
        {
            var fieldsAsString = DictionaryToJsonString(customFields);
...
#if !UNITY_IOS && !UNITY_TVOS
        public static void AddBusinessEvent(string currency, int amount, string itemType, string itemId,
            string cartType, IDictionary<string, object> fields, bool mergeFields)
        {
            var fieldsAsString = DictionaryToJsonString(fields);
#if UNITY_EDITOR
        ...
        public static void AddResourceEvent(GAResourceFlowType flowType, string currency, float amount, string itemType,
            string itemId, IDictionary<string, object> fields, bool mergeFields)
        {
            var fieldsAsString = DictionaryToJsonString(fields);
#if UNITY_EDITOR
        ...
        public static void AddProgressionEvent(GAProgressionStatus progressionStatus, string progression01,
            string progression02, string progression03, IDictionary<string, object> fields, bool mergeFields)
        {
            var fieldsAsString = DictionaryToJsonString(fields);
#if UNITY_EDITOR
            if (GAValidator.ValidateProgressionEvent(progressionStatus, progression01, progression02, progression03))
                addProgressionEvent((int) progressionStatus, progression01, progression02, progression03,
                    fieldsAsString, mergeFields);
#else
                addProgressionEvent ((int)progressionStatus, progression01, progression02, progression03, fieldsAsString, mergeFields);
#endif
        }

        public static void AddProgressionEventWithScore(GAProgressionStatus progressionStatus, string progression01,
            string progression02, string progression03, int score, IDictionary<string, object> fields, bool mergeFields)
        {
            var fieldsAsString = DictionaryToJsonString(fields);
#if UNITY_EDITOR
            if (GAValidator.ValidateProgressionEvent(progressionStatus, progression01, progression02, progression03))
                addProgressionEventWithScore((int) progressionStatus, progression01, progression02, progression03,
                    score, fieldsAsString, mergeFields);
#else
        ...
        public static void AddDesignEvent(string eventID, IDictionary<string, object> fields, bool mergeFields)
        {
            var fieldsAsString = DictionaryToJsonString(fields);
#if UNITY_EDITOR
            if (GAValidator.ValidateDesignEvent(eventID)) addDesignEvent(eventID, fieldsAsString, mergeFields);
#else
        ...
        public static void AddAdEventWithDuration(GAAdAction adAction, GAAdType adType, string adSdkName,
            string adPlacement, long duration, IDictionary<string, object> fields, bool mergeFields)
        {
            var fieldsAsString = DictionaryToJsonString(fields);
#if UNITY_EDITOR
        ...
        public static void AddAdEventWithReason(GAAdAction adAction, GAAdType adType, string adSdkName,
            string adPlacement, GAAdError noAdReason, IDictionary<string, object> fields, bool mergeFields)
        {
            var fieldsAsString = DictionaryToJsonString(fields);
#if UNITY_EDITOR
            if (GAValidator.ValidateAdEvent(adAction, adType, adSdkName, adPlacement))
        ...
        public static void AddAdEvent(GAAdAction adAction, GAAdType adType, string adSdkName, string adPlacement,
            IDictionary<string, object> fields, bool mergeFields)
        {
            var fieldsAsString = DictionaryToJsonString(fields);
#if UNITY_EDITOR
            if (GAValidator.ValidateAdEvent(adAction, adType, adSdkName, adPlacement))
