...
# endif
        ...
        private static void InternalInitialize()
        {
            ...
            if (SettingsGA.InfoLogBuild) GA_Setup.SetInfoLog(true);

            if (SettingsGA.VerboseLogBuild) GA_Setup.SetVerboseLog(true);

            var platformIndex = GetPlatformIndex();
            ...
            if (platformIndex >= 0)
            {
                ...
                {
                    for (var i = 0; i < SettingsGA.Platforms.Count; ++i)
                        if (SettingsGA.Platforms[i] == RuntimePlatform.Android ||
                            SettingsGA.Platforms[i] == RuntimePlatform.IPhonePlayer)
                            SettingsGA.Build[i] = Application.version;
                    if (SettingsGA.Platforms[platformIndex] == RuntimePlatform.Android ||
                        SettingsGA.Platforms[platformIndex] == RuntimePlatform.IPhonePlayer)
            ...
            if (SettingsGA.UseManualSessionHandling) SetEnabledManualSessionHandling(true);
        }
        ...
        public static void NewBusinessEvent(string currency, int amount, string itemType, string itemId,
            string cartType, IDictionary<string, object> customFields, bool mergeFields = false)
        {
            if (!_hasInitializeBeenCalled)
            {
                Debug.LogError("GameAnalytics: REMEMBER THE SDK NEEDS TO BE MANUALLY INITIALIZED NOW");
        ...
        /// <param name="receipt">Transaction receipt string.</param>
        /// <param name="customFields">Custom fields to add to the event. Dictionary of key-value pairs. Only string or numbers allowed as values. Custom fields are only stored in raw events and can only be used for data export (i.e. not visible in the tool).</param>
        public static void NewBusinessEventIOS(string currency, int amount, string itemType, string itemId, string cartType, string receipt, IDictionary<string, object> customFields, bool mergeFields
 = false)
        {
            if (!GameAnalytics._hasInitializeBeenCalled)
            {
        ...
        /// <param name="cartType">Cart type.</param>
        /// <param name="customFields">Custom fields to add to the event. Dictionary of key-value pairs. Only string or numbers allowed as values. Custom fields are only stored in raw events and can only be used for data export (i.e. not visible in the tool).</param>
        public static void NewBusinessEventIOSAutoFetchReceipt(string currency, int amount, string itemType, string itemId, string cartType, IDictionary<string, object> customFields, bool mergeFields
 = false)
        {
            if (!GameAnalytics._hasInitializeBeenCalled)
            {
        ...
        /// <param name="signature">Signature of transaction.</param>
        /// <param name="customFields">Custom fields to add to the event. Dictionary of key-value pairs. Only string or numbers allowed as values. Custom fields are only stored in raw events and can only be used for data export (i.e. not visible in the tool).</param>
        public static void NewBusinessEventGooglePlay(string currency, int amount, string itemType, string itemId, string cartType, string receipt, string signature, IDictionary<string, object> customFields, bool mergeFields
 = false)
        {
            if (!GameAnalytics._hasInitializeBeenCalled)
            {
        ...
        /// <param name="eventName">String can consist of 1 to 5 segments. Segments are seperated by ':' and segments can have a max length of 16. (e.g. segment1:anotherSegment:gold).</param>
        /// <param name="customFields">Custom fields to add to the event. Dictionary of key-value pairs. Only string or numbers allowed as values. Custom fields are only stored in raw events and can only be used for data export (i.e. not visible in the tool).</param>
        public static void NewDesignEvent(string eventName, IDictionary<string, object> customFields,
        ...
        /// <param name="eventValue">Number value of event.</param>
        /// <param name="customFields">Custom fields to add to the event. Dictionary of key-value pairs. Only string or numbers allowed as values. Custom fields are only stored in raw events and can only be used for data export (i.e. not visible in the tool).</param>
        public static void NewDesignEvent(string eventName, float eventValue, IDictionary<string, object> customFields,
            bool mergeFields = false)
        {
            if (!_hasInitializeBeenCalled)
            {
        ...
        /// <param name="customFields">Custom fields to add to the event. Dictionary of key-value pairs. Only string or numbers allowed as values. Custom fields are only stored in raw events and can only be used for data export (i.e. not visible in the tool).</param>
        public static void NewProgressionEvent(GAProgressionStatus progressionStatus, string progression01,
            IDictionary<string, object> customFields, bool mergeFields = false)
        {
            if (!_hasInitializeBeenCalled)
            {
                Debug.LogError("GameAnalytics: REMEMBER THE SDK NEEDS TO BE MANUALLY INITIALIZED NOW");
        ...
        /// <param name="customFields">Custom fields to add to the event. Dictionary of key-value pairs. Only string or numbers allowed as values. Custom fields are only stored in raw events and can only be used for data export (i.e. not visible in the tool).</param>
        public static void NewProgressionEvent(GAProgressionStatus progressionStatus, string progression01,
            string progression02, IDictionary<string, object> customFields, bool mergeFields = false)
        {
            if (!_hasInitializeBeenCalled)
            {
                Debug.LogError("GameAnalytics: REMEMBER THE SDK NEEDS TO BE MANUALLY INITIALIZED NOW");
        ...
        /// <param name="customFields">Custom fields to add to the event. Dictionary of key-value pairs. Only string or numbers allowed as values. Custom fields are only stored in raw events and can only be used for data export (i.e. not visible in the tool).</param>
        public static void NewProgressionEvent(GAProgressionStatus progressionStatus, string progression01,
            string progression02, string progression03, IDictionary<string, object> customFields,
            bool mergeFields = false)
        {
            if (!_hasInitializeBeenCalled)
            {
            ...
            GA_Progression.NewEvent(progressionStatus, progression01, progression02, progression03, customFields,
                mergeFields);
        }
        ...
        /// <param name="customFields">Custom fields to add to the event. Dictionary of key-value pairs. Only string or numbers allowed as values. Custom fields are only stored in raw events and can only be used for data export (i.e. not visible in the tool).</param>
        public static void NewProgressionEvent(GAProgressionStatus progressionStatus, string progression01,
            string progression02, int score, IDictionary<string, object> customFields, bool mergeFields = false)
        {
            if (!_hasInitializeBeenCalled)
            {
                Debug.LogError("GameAnalytics: REMEMBER THE SDK NEEDS TO BE MANUALLY INITIALIZED NOW");
        ...
        /// <param name="customFields">Custom fields to add to the event. Dictionary of key-value pairs. Only string or numbers allowed as values. Custom fields are only stored in raw events and can only be used for data export (i.e. not visible in the tool).</param>
        public static void NewProgressionEvent(GAProgressionStatus progressionStatus, string progression01,
            string progression02, string progression03, int score, IDictionary<string, object> customFields,
            bool mergeFields = false)
        {
            if (!_hasInitializeBeenCalled)
            {
            ...
            GA_Progression.NewEvent(progressionStatus, progression01, progression02, progression03, score, customFields,
                mergeFields);
        }
        ...
        /// <param name="customFields">Custom fields to add to the event. Dictionary of key-value pairs. Only string or numbers allowed as values. Custom fields are only stored in raw events and can only be used for data export (i.e. not visible in the tool).</param>
        public static void NewResourceEvent(GAResourceFlowType flowType, string currency, float amount, string itemType,
            string itemId, IDictionary<string, object> customFields, bool mergeFields = false)
        {
            if (!_hasInitializeBeenCalled)
            {
                Debug.LogError("GameAnalytics: REMEMBER THE SDK NEEDS TO BE MANUALLY INITIALIZED NOW");
        ...
        /// <param name="customFields">Custom fields to add to the event. Dictionary of key-value pairs. Only string or numbers allowed as values. Custom fields are only stored in raw events and can only be used for data export (i.e. not visible in the tool).</param>
        public static void NewAdEvent(GAAdAction adAction, GAAdType adType, string adSdkName, string adPlacement,
            long duration, IDictionary<string, object> customFields, bool mergeFields = false)
        {
            if (!_hasInitializeBeenCalled)
            {
                Debug.LogError("GameAnalytics: REMEMBER THE SDK NEEDS TO BE MANUALLY INITIALIZED NOW");
        ...
        /// <param name="customFields">Custom fields to add to the event. Dictionary of key-value pairs. Only string or numbers allowed as values. Custom fields are only stored in raw events and can only be used for data export (i.e. not visible in the tool).</param>
        public static void NewAdEvent(GAAdAction adAction, GAAdType adType, string adSdkName, string adPlacement,
            GAAdError noAdReason, IDictionary<string, object> customFields, bool mergeFields = false)
        {
            if (!_hasInitializeBeenCalled)
            {
                Debug.LogError("GameAnalytics: REMEMBER THE SDK NEEDS TO BE MANUALLY INITIALIZED NOW");
        ...
        {
            if (OnRemoteConfigsUpdatedEvent != null) OnRemoteConfigsUpdatedEvent();
        }
        ...
        {
            var unityVersion = "";
            var splitUnityVersion = Application.unityVersion.Split('.');
            for (var i = 0; i < splitUnityVersion.Length; i++)
            {
                int result;
                ...
                {
                    var regexVersion = System.Text.RegularExpressions.Regex.Split(splitUnityVersion[i], "[^\\d]+");
        ...
        public static string WhereIs(string _file, string _type)
        {
            ...
            return "";
#else
            var guids = AssetDatabase.FindAssets("t:" + _type);
            foreach (var g in guids)
            {
                var p = AssetDatabase.GUIDToAssetPath(g);
                if (p.EndsWith(_file)) return p;
            }
        ...
        public static void HierarchyWindowCallback(int instanceID, Rect selectionRect)
        {
            ...
            if (go != null && go.GetComponent<GameAnalytics>() != null)
            {
                ...
                if (SettingsGA.Logo == null)
                    SettingsGA.Logo =
                        (Texture2D) AssetDatabase.LoadAssetAtPath(WhereIs("gaLogo.png", "Texture2D"),
                            typeof(Texture2D));

                Graphics.DrawTexture(
                    new Rect(GUILayoutUtility.GetLastRect().width - selectionRect.height - 5 - addX, selectionRect.y,
                        selectionRect.height, selectionRect.height), SettingsGA.Logo);
            }
        }
        ...
        {
            for (var i = 0; i < SettingsGA.Build.Count; i++) SettingsGA.Build[i] = build;
        }
    }
}