...
namespace GameAnalyticsSDK.Events
{
    public static class GA_Debug
    {
        ...
        public static void HandleLog(string logString, string stackTrace, LogType type)
        {
            ...
            if (GameAnalytics.SettingsGA != null && GameAnalytics.SettingsGA.SubmitErrors &&
                _errorCount < MaxErrorCount && type != LogType.Log)
            {
                if (string.IsNullOrEmpty(stackTrace)) stackTrace = "";
                _errorCount++;

                var lString = logString.Replace('"', '\'').Replace('\n', ' ').Replace('\r', ' ');
                var sTrace = stackTrace.Replace('"', '\'').Replace('\n', ' ').Replace('\r', ' ');

                var _message = lString + " " + sTrace;
                if (_message.Length > 8192) _message = _message.Substring(0, 8191);
        ...
        {
            var severity = GAErrorSeverity.Info;
