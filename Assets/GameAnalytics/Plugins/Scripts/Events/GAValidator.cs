...
namespace GameAnalyticsSDK.Validators
{
    internal static class GAValidator
    {
        ...
        public static bool ValidateBusinessEvent(string currency, int amount, string cartType, string itemType,
            string itemId)
        {
            // validate currency
            ...
            {
                Debug.Log("Validation fail - business event - cartType. Cannot be above 32 length. String: " +
                          cartType);
                return false;
            }
        ...
        public static bool ValidateResourceEvent(GAResourceFlowType flowType, string currency, float amount,
            string itemType, string itemId)
        {
            ...
            {
                Debug.Log(
                    "Validation fail - resource event - currency: Not found in list of pre-defined resource currencies. String: " +
                    currency);
                return false;
            }
            ...
            if (!(amount > 0))
            {
                Debug.Log("Validation fail - resource event - amount: Float amount cannot be 0 or negative. Value: " +
                          amount);
                return false;
            }
        ...
        public static bool ValidateProgressionEvent(GAProgressionStatus progressionStatus, string progression01,
            string progression02, string progression03)
        {
            ...
            if (!string.IsNullOrEmpty(progression03) &&
                !(!string.IsNullOrEmpty(progression02) || string.IsNullOrEmpty(progression01)))
            {
                Debug.Log(
        ...
        {
            if (!ValidateKeys(gameKey, gameSecret)) return false;
        ...
        public static bool ValidateCurrency(string currency)
        {
            ...
            if (!StringMatch(currency, "^[A-Z]{3}$")) return false;
            return true;
        }

        public static bool ValidateEventPartLength(string eventPart, bool allowNull)
        {
            if (allowNull == true && string.IsNullOrEmpty(eventPart)) return true;

            if (string.IsNullOrEmpty(eventPart)) return false;

            if (eventPart.Length > 64) return false;
            return true;
        }

        public static bool ValidateEventPartCharacters(string eventPart)
        {
            if (!StringMatch(eventPart, "^[A-Za-z0-9\\s\\-_\\.\\(\\)\\!\\?]{1,64}$")) return false;
            return true;
        }
        ...
        public static bool ValidateEventIdLength(string eventId)
        {
            ...
            if (!StringMatch(eventId, "^[^:]{1,64}(?::[^:]{1,64}){0,4}$")) return false;
            return true;
        }
        ...
        {
            if (!ValidateShortString(build, false)) return false;
            return true;
        }
        ...
        public static bool ValidateShortString(string shortString, bool canBeEmpty)
        {
            ...
            if (canBeEmpty && string.IsNullOrEmpty(shortString)) return true;

            if (string.IsNullOrEmpty(shortString) || shortString.Length > 32) return false;
            return true;
        }
        ...
        {
            // String is allowed to be empty or nil
            if (canBeEmpty && string.IsNullOrEmpty(s)) return true;
        ...
        public static bool ValidateLongString(string longString, bool canBeEmpty)
        {
            ...
            if (canBeEmpty && string.IsNullOrEmpty(longString)) return true;

            if (string.IsNullOrEmpty(longString) || longString.Length > 8192) return false;
            return true;
        }
        ...
        {
            if (!ValidateArrayOfStrings(20, 64, false, "resource currencies", resourceCurrencies)) return false;

            // validate each string for regex
            foreach (var resourceCurrency in resourceCurrencies)
        ...
        {
            if (!ValidateArrayOfStrings(20, 32, false, "resource item types", resourceItemTypes)) return false;

            // validate each resourceItemType for eventpart validation
            foreach (var resourceItemType in resourceItemTypes)
        ...
        public static bool ValidateDimension01(string dimension01)
        {
            ...
            {
                Debug.Log(
                    "Validation failed - custom dimension 01 - value was not found in list of custom dimensions 01 in the Settings object. \nGiven dimension value: " +
                    dimension01);
                return false;
            }
        ...
        public static bool ValidateDimension02(string dimension02)
        {
            ...
            {
                Debug.Log(
                    "Validation failed - custom dimension 02 - value was not found in list of custom dimensions 02 in the Settings object. \nGiven dimension value: " +
                    dimension02);
                return false;
            }
        ...
        public static bool ValidateDimension03(string dimension03)
        {
            ...
            {
                Debug.Log(
                    "Validation failed - custom dimension 03 - value was not found in list of custom dimensions 03 in the Settings object. \nGiven dimension value: " +
                    dimension03);
                return false;
            }
        ...
        public static bool ValidateArrayOfStrings(long maxCount, long maxStringLength, bool allowNoValues,
            string logTag, params string[] arrayOfStrings)
        {
            var arrayTag = logTag;

            // use arrayTag to annotate warning log
            if (string.IsNullOrEmpty(arrayTag)) arrayTag = "Array";
            ...
            {
                Debug.Log(arrayTag + " validation failed: array cannot exceed " + maxCount + " values. It has " +
                          arrayOfStrings.Length + " values.");
                return false;
            }

            // validate each string
            foreach (var arrayString in arrayOfStrings)
            {
                var stringLength = arrayString == null ? 0 : arrayString.Length;
                ...
                {
                    Debug.Log(arrayTag + " validation failed: a string exceeded max allowed length (which is: " +
                              maxStringLength + "). String was: " + arrayString);
                    return false;
                }
            }
        ...
        public static bool ValidateClientTs(long clientTs)
        {
            ...
            if (clientTs < long.MinValue + 1 || clientTs > long.MaxValue - 1) return false;
            return true;
        }
