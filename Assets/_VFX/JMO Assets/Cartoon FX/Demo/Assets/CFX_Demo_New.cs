...
public class CFX_Demo_New : MonoBehaviour
{
    ...
    private void Awake()
    {
        var particleExampleList = new List<GameObject>();
        var nbChild = transform.childCount;
    ...
    private void Update()
    {
        if (Input.GetKeyDown(KeyCode.LeftArrow))
            ...
            nextParticle();
        else if (Input.GetKeyDown(KeyCode.Delete)) destroyParticles();
        ...
        if (scroll != 0f) Camera.main.transform.Translate(Vector3.forward * (scroll < 0f ? -1f : 1f), Space.Self);
...
//				particles.transform.GetChild(i).gameObject.SetActive(true);
    ...
    private IEnumerator CheckForDeletedParticles()
    {
        while (true)
        {
            ...
            for (var i = onScreenParticles.Count - 1; i >= 0; i--)
    ...
    private void destroyParticles()
    {
        for (var i = onScreenParticles.Count - 1; i >= 0; i--)
        {
            if (onScreenParticles[i] != null) Destroy(onScreenParticles[i]);
