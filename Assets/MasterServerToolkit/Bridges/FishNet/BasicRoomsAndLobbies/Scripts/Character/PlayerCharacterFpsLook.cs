...
namespace MasterServerToolkit.Bridges.FishNetworking.Character
{
    ...
    public class PlayerCharacterFpsLook : PlayerCharacterLook
    {
        ...
        protected override void UpdateCameraRotation()
        {
            ...
            cameraRotation.x =
 Mathf.Clamp(cameraRotation.x - inputController.MouseY() * lookSensitivity.y, minLookAngle, maxLookAngle);

            if (useSmoothness)
            {
                transform.rotation = Quaternion.Euler(0f, smoothedCameraRotation.y, 0f);
                smoothedCameraRotation =
 Vector3.SmoothDamp(smoothedCameraRotation, cameraRotation, ref currentCameraRotationVelocity, smoothnessTime);
                lookCamera.transform.rotation =
 Quaternion.Euler(smoothedCameraRotation.x, smoothedCameraRotation.y, 0f);
            }
            else
            {
