...
namespace MasterServerToolkit.Bridges.FishNetworking.Character
{
        ...
        protected override void UpdateMovement()
        {
            ...
            if (characterController.isGrounded)
            {
                ...
                if (inputController.IsMoving() && !inputController.IsArmed())
                {
                    ...
                    {
                        playerTargetDirectionAngle =
 Quaternion.LookRotation(currentDirection) * lookController.GetCameraRotation();
                    }
                }
                ...
                {
                    playerTargetDirectionAngle =
 Quaternion.LookRotation(new Vector3(aimDirection.x, 0f, aimDirection.z));
                }
 ...
 Quaternion.LookRotation(new Vector3(aimDirection.x, 0f, aimDirection.z));
                ...
                if (movementIsAllowed)
                {
                    ...
                    transform.rotation =
 Quaternion.Lerp(transform.rotation, playerTargetDirectionAngle, Time.deltaTime * rotationSmoothTime);
                }

                // Let's calculate input direction
                var inputAxisAngle =
 inputController.MovementAxisDirection().Equals(Vector3.zero) ? Vector3.zero : Quaternion.LookRotation(inputController.MovementAxisDirection()).eulerAngles;

                //
                var compositeAngle = inputAxisAngle - transform.eulerAngles;

                // 
                calculatedInputDirection =
 Quaternion.Euler(compositeAngle) * lookController.GetCameraRotation() * transform.forward * inputController.MovementAxisMagnitude();
