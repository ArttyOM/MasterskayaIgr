...
namespace MasterServerToolkit.Bridges.LiteDB
{
    public class AccountsDatabaseAccessor : IAccountsDatabaseAccessor
    {
        ...
        public MstProperties CustomProperties { get; private set; } = new();
        ...
        public async Task<IAccountInfoData> GetAccountByUsernameAsync(string username)
        {
            ...
            await Task.Run(() => { account = accountsCollection.FindOne(a => a.Username == username); });
        ...
        public async Task<IAccountInfoData> GetAccountByTokenAsync(string token)
        {
            ...
            await Task.Run(() => { account = accountsCollection.FindOne(a => a.Token == token); });
        ...
        public async Task<IAccountInfoData> GetAccountByEmailAsync(string email)
        {
            ...
            await Task.Run(() => { account = accountsCollection.FindOne(i => i.Email == email.ToLower()); });
        ...
        {
            return await Task.Run(() => { return accountsCollection.FindOne(i => i.Id == id.ToLower()); });
        }

        public async Task<IAccountInfoData> GetAccountByDeviceIdAsync(string deviceId)
        {
            return await Task.Run(() => { return accountsCollection.FindOne(i => i.DeviceId == deviceId.ToLower()); });
        }
        ...
        public async Task<string> GetPasswordResetDataAsync(string email)
        {
            ...
            await Task.Run(() => { data = resetCodesCollection.FindOne(i => i.Email == email.ToLower()); });
        ...
        public async Task<bool> UpdateAccountAsync(IAccountInfoData account)
        {
            ...
            if (!string.IsNullOrEmpty(username))
            {
                ...
                if (existingAccount != null && existingAccount.Id != account.Id)
                    throw new Exception(
                        "You are trying to update your username but it is already taken by another user");
            }
