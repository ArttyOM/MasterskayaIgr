...
namespace MasterServerToolkit.Bridges.LiteDB
{
    public class AccountInfoData : IAccountInfoData
    {
        ...
        {
            Properties.TryGetValue(MstDictKeys.USER_AUTH_TOKEN_EXPIRES, out var expireTime);

            if (!string.IsNullOrEmpty(expireTime))
            {
                var filetime = Convert.ToInt64(expireTime);
        ...
        public void SetToken(int tokenExpiresInDays)
        {
            ...
            Properties[MstDictKeys.USER_AUTH_TOKEN_EXPIRES] =
                DateTime.Now.AddDays(tokenExpiresInDays).ToFileTime().ToString();
        }
