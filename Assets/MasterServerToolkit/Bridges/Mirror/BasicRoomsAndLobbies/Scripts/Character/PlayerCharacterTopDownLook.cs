...
namespace MasterServerToolkit.Bridges.MirrorNetworking.Character
{
 ...
 Vector3.Lerp(cameraRoot.transform.position, cameraOffsetPosition + lookAtPoint, Time.deltaTime * followSmoothTime);
            }
            else
            {
        ...
        protected override void UpdateCameraRotation()
        {
            ...
            float newAngle =
 Mathf.LerpAngle(cameraRoot.transform.rotation.eulerAngles.y, currentCameraYawAngle, Time.deltaTime * 4f);
 ...
 Vector3.Lerp(lookCamera.transform.localPosition, new Vector3(0f, 0f, (cameraCollisionDistance - 0.5f) * -1f), Time.deltaTime * collisionDstanceSmoothTime);
            }
            else
            {
                currentCameraDistance =
 Mathf.Clamp(currentCameraDistance - (inputController.MouseVerticalScroll() * distanceScrollPower), minDistance, maxDistance);
                lookCamera.transform.localPosition =
 Vector3.Lerp(lookCamera.transform.localPosition, new Vector3(0f, 0f, currentCameraDistance * -1f), Time.deltaTime * distanceSmoothTime);
            }
        }
