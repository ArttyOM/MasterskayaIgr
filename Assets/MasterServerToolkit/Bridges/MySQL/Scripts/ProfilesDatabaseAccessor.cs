            ...
            try
            {
                using (var connection = new MySqlConnection(connectionString))
                {
                    ...
                    using (var cmd = new MySqlCommand())
                    {
                        ...
                        cmd.CommandText =
                            $"SELECT `property_key`, `property_value` FROM profiles WHERE account_id = '{profile.UserId}';";
                        ...
                        using (var reader = (MySqlDataReader) await cmd.ExecuteReaderAsync())
                        {
                            if (reader.HasRows)
                                ...
                                {
                                    var propertyKey = reader.GetString("property_key");

                                    if (profile.TryGet(propertyKey.ToUint16Hash(), out IObservableProperty property))
                                    {
                                        var propertyValue = reader.GetString("property_value");
            ...
            using (var con = new MySqlConnection(connectionString))
            {
                using (var cmd = new MySqlCommand())
                {
                    ...
                    foreach (var property in profile)
                    {
                        index++;
                        sql.Append(
                            $"('{profile.UserId}','{Extensions.StringExtensions.FromHash(property.Key)}','{property.ToJson()}'){(index < profile.Count ? "," : "")} ");
                    }

                    sql.Append($"as p ");
                    sql.Append(
                        $"ON DUPLICATE KEY UPDATE account_id=p.account_id, property_key=p.property_key, property_value=p.property_value;");
