...
namespace MasterServerToolkit.Bridges
{
    public class PasswordResetView : UIView
    {
        ...
        [SerializeField] private TMP_InputField newPasswordConfirmInputField;

        public string ResetCode => resetCodeInputField != null ? resetCodeInputField.text : string.Empty;

        public string NewPassword => newPasswordInputField != null ? newPasswordInputField.text : string.Empty;

        public string NewPasswordConfirm =>
            newPasswordConfirmInputField != null ? newPasswordConfirmInputField.text : string.Empty;
        ...
        public void ResetPassword()
        {
            ...
            if (AuthBehaviour.Instance)
                AuthBehaviour.Instance.ResetPassword(Mst.Options.AsString(MstDictKeys.RESET_PASSWORD_EMAIL), ResetCode,
                    NewPassword);
            else
                logger.Error(
                    $"No instance of {nameof(AuthBehaviour)} found. Please add {nameof(AuthBehaviour)} to scene to be able to use auth logic");
        }
