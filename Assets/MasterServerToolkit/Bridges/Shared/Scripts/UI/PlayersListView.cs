...
namespace MasterServerToolkit.Bridges
{
    public class PlayersListView : UIView
    {
        ...
        public void FindPlayers()
        {
            ...
            if (roomId < 0 && Mst.Client.Rooms.HasAccess) roomId = Mst.Client.Rooms.ReceivedAccess.RoomId;
        ...
        private void DrawPlayersList(GameInfoPacket game)
        {
            if (listContainer)
            {
                ...
                foreach (var player in game.OnlinePlayersList)
                {
                    var isMasterUser = game.Properties.AsString("-room.masterUser") == player;
