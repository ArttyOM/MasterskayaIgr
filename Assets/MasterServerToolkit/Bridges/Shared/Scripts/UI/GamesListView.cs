...
namespace MasterServerToolkit.Bridges
{
    public class GamesListView : UIView
    {
        ...
        [SerializeField] private UILable uiColLablePrefab;
        ...
        private void DrawGamesList(IEnumerable<GameInfoPacket> games)
        {
            if (listContainer)
            {
                ...
                foreach (var gameInfo in games)
                {
                    ...
                    gameNameLable.Text = gameInfo.IsPasswordProtected
                    ...
                    var rx = new Regex(@":\d+");
                    var ip = rx.Replace(gameInfo.Address.Trim(), "");

                    MstTimer.WaitPing(ip, (time) => { pingRegionLable.Text = $"{time} ms."; });
                    ...
                    gameConnectBtn.AddOnClickListener(() => { MatchmakingBehaviour.Instance.StartMatch(gameInfo); });
