...
namespace MasterServerToolkit.Bridges
{
    public class ChatsView : UIView
    {
        ...
        [SerializeField] private RectTransform chatMesagesContainer;
        [SerializeField] private TMP_Text statusInfoText;
        [SerializeField] private TMP_Text chatTitleText;
        [SerializeField] private ChatMessageItemUI incomingMessageItemPrefab;
        [SerializeField] private ChatMessageItemUI outgoingMessageItemPrefab;
        [SerializeField] private ChatChannelItemUI chatChannelItemPrefab;
        [SerializeField] private TMP_InputField messageInputField;

        [Header("Settings")] [SerializeField] private string defaultChannelName = "MST Chat Demo";
        ...
        private void Update()
        {
            if (Input.GetKeyDown(KeyCode.Return)) TrySendMessage();
        }
        ...
        public void TrySendMessage()
        {
            if (!string.IsNullOrEmpty(messageInputField.text))
            {
                if (string.IsNullOrEmpty(username))
                {
                    Mst.Events.Invoke(MstEventKeys.showOkDialogBox,
                        new OkDialogBoxEventMessage(
                            "Username cannot be empty! Set username in UsernamePickView and try again",
                ...
                var message = new ChatMessagePacket()
                {
                    ...
                    Message = messageInputField.text.Length > 200
                        ? $"{messageInputField.text.Substring(0, 200)}..."
        ...
        {
            foreach (Transform t in chatMesagesContainer) Destroy(t.gameObject);
        }
        ...
        {
            foreach (Transform t in chatChannelsContainer) Destroy(t.gameObject);
        }
