...
namespace MasterServerToolkit.Bridges
{
    public class AuthBehaviour : BaseClientBehaviour
    {
        ...
        [Header("Settings")] [SerializeField] protected bool rememberUser = true;

        [Header("Editor Settings")] [SerializeField]
        protected string defaultUsername = "qwerty";

        [SerializeField] protected string defaultEmail = "qwerty@mail.com";
        [SerializeField] protected string defaultPassword = "qwerty123!@#";
        [SerializeField] protected bool useDefaultCredentials = false;
                ...
                Mst.Client.Auth.SignInWithLoginAndPassword(username, password, (accountInfo, error) =>
                {
                    ...
                    if (accountInfo != null)
                    {
                        if (accountInfo.IsEmailConfirmed)
                            ...
                            Mst.Events.Invoke(MstEventKeys.showEmailConfirmationView,
                                Mst.Client.Auth.AccountInfo.Email);

                        Mst.Events.Invoke(MstEventKeys.hideSignInView);
                    }
                    else
                    {
                        var outputMessage = $"{Mst.Localization["signInErrorResult"]} {error}";
                        logger.Error(outputMessage);

                        Mst.Events.Invoke(MstEventKeys.showOkDialogBox,
                            new OkDialogBoxEventMessage(outputMessage,
                                () => { Mst.Events.Invoke(MstEventKeys.showSignInView); }));
                    }
                }, Connection);
            });
        ...
        public virtual void SignUp(string username, string useremail, string userpassword)
        {
            ...
            MstTimer.WaitForSeconds(0.1f, () =>
            {
                Mst.Client.Auth.SignUp(credentials, (isSuccessful, error) =>
                {
                    ...
                    {
                        var outputMessage = Mst.Localization["signUpErrorResult"];
                        logger.Error(outputMessage);
            ...
            MstTimer.WaitForSeconds(0.1f, () =>
            {
                Mst.Client.Auth.ChangePassword(userEmail, resetCode, newPassword, (isSuccessful, error) =>
                {
                    ...
                    if (isSuccessful)
                    {
                        ...
                        Mst.Events.Invoke(MstEventKeys.showSignInView);
                        Mst.Events.Invoke(MstEventKeys.showOkDialogBox,
                            new OkDialogBoxEventMessage(Mst.Localization["changePasswordProgress"], null));
                    }
                    else
                    {
                        var outputMessage = $"{Mst.Localization["changePasswordErrorResult"]} {error}";
                        logger.Error(outputMessage);
            ...
            MstTimer.WaitForSeconds(0.1f, () =>
            {
                Mst.Client.Auth.RequestPasswordReset(userEmail, (isSuccessful, error) =>
                {
                    ...
                    if (isSuccessful)
                    {
                        ...
                        Mst.Events.Invoke(MstEventKeys.showPasswordResetView);
                        Mst.Events.Invoke(MstEventKeys.showOkDialogBox,
                            new OkDialogBoxEventMessage(
                                $"{Mst.Localization["changePasswordCodeSuccessResult"]} '{userEmail}'", null));
                    }
                    else
                    {
                        var outputMessage = $"{Mst.Localization["changePasswordCodeErrorResult"]} {error}";
                        logger.Error(outputMessage);
            ...
            MstTimer.WaitForSeconds(0.1f, () =>
            {
                Mst.Client.Auth.RequestEmailConfirmationCode((isSuccessful, error) =>
                {
                    ...
                    if (isSuccessful)
                    {
                        Mst.Events.Invoke(MstEventKeys.showOkDialogBox,
                            new OkDialogBoxEventMessage(
                                $"{Mst.Localization["confirmationCodeSendingSuccessResult"]} '{Mst.Client.Auth.AccountInfo.Email}'",
                                null));
                    }
                    else
                    {
                        var outputMessage = $"{Mst.Localization["confirmationCodeSendingErrorResult"]}: {error}";
                        Mst.Events.Invoke(MstEventKeys.showOkDialogBox,
                            new OkDialogBoxEventMessage(outputMessage, null));
                        logger.Error(outputMessage);
                    }
                });
            });
        ...
        public virtual void ConfirmAccount(string confirmationCode)
        {
            ...
            MstTimer.WaitForSeconds(0.1f, () =>
            {
                Mst.Client.Auth.ConfirmEmail(confirmationCode, (isSuccessful, error) =>
                {
                    ...
                    {
                        var outputMessage = $"{Mst.Localization["accountConfirmationErrorResult"]} {error}";
                        Mst.Events.Invoke(MstEventKeys.showOkDialogBox,
                            new OkDialogBoxEventMessage(outputMessage, null));
                        logger.Error(outputMessage);
                    }
