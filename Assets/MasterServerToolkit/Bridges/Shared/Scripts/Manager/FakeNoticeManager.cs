...
namespace MasterServerToolkit.Bridges
{
    public class FakeNoticeManager : MonoBehaviour
    {
        ...
        [Header("Settings")] [SerializeField] private bool active = true;
        ...
        {
            using (var www = UnityWebRequest.Get("https://jsonplaceholder.typicode.com/users"))
            {
        ...
        {
            using (var www = UnityWebRequest.Get("https://jsonplaceholder.typicode.com/todos"))
            {
                yield return www.SendWebRequest();
        ...
        {
            var timeToWait = Random.Range(3f, 10f);
                ...
                if (isSuccess)
                    MstTimer.WaitForRealtimeSeconds(timeToWait, () =>
                    {
                        ...
                        var user = users.Values
                            .Where(i => i.GetField("id").StringValue == task.GetField("userId").StringValue)
                            .FirstOrDefault();

                        if (user != null)
                        {
                            var message = $"<b>{user["username"]}</b>\n{task["title"]}";

                            if (roomServerManager.HasPlayers)
                            {
                                Mst.Server.Notifications.NotifyRoom(roomServerManager.RoomController.RoomId, message,
                                    null, roomServerManager.Connection);
                                StartTaskNotices();
                            }
                        }
                        else
