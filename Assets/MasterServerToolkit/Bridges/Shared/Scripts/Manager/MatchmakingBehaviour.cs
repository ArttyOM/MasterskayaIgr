...
namespace MasterServerToolkit.Bridges
{
    public class MatchmakingBehaviour : BaseClientBehaviour
    {
        ...
        /// </summary>
        public MstProperties CustomRoomProperties { get; private set; } = new();
            ...
            UnityAction failCallback = null)
        {
            ...
            customSpawnOptions.Add("-room.masterUser",
                Mst.Client.Auth.IsSignedIn ? Mst.Client.Auth.AccountInfo.Username : "Anonymous");
            ...
            Mst.Client.Spawners.RequestSpawn(spawnOptions, customSpawnOptions, regionName, (controller, error) =>
            {
                if (controller == null)
                {
                    ...
                    Mst.Events.Invoke(MstEventKeys.showOkDialogBox,
                        new OkDialogBoxEventMessage(error, () => { failCallback?.Invoke(); }));
                ...
                MstTimer.WaitWhile(() => { return !roomStartingProcessCompleted; }, (isSuccess) =>
                {
                    controller.OnStatusChangedEvent -= Controller_OnStatusChangedEvent;
                    ...
                    if (isSuccess)
                    {
                        ...
                        else
                        {
                            ...
                            Mst.Events.Invoke(MstEventKeys.showOkDialogBox,
                                new OkDialogBoxEventMessage("Failed spawn new room. Please, try later",
                                    () => { failCallback?.Invoke(); }));
                        }
                    }
                    else
                    {
                        ...
                        Mst.Events.Invoke(MstEventKeys.showOkDialogBox,
                            new OkDialogBoxEventMessage("Failed spawn new room. Time out",
                                () => { failCallback?.Invoke(); }));
                    }
        ...
        public virtual void StartMatch(GameInfoPacket gameInfo)
        {
            ...
            if (gameInfo.IsPasswordProtected)
                Mst.Events.Invoke(MstEventKeys.showPasswordDialogBox,
                    new PasswordInputDialoxBoxEventMessage(
                        "Room requires the password. Please enter room password below", () =>
                        {
                            // Get password if was set
                            var password = Mst.Options.AsString(Mst.Args.Names.RoomPassword);
