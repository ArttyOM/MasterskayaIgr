...
namespace MasterServerToolkit.Logging
{
    public class LogManager
    {
        ...
        private static readonly Queue<PooledLog> _pooledLogs = new();
        ...
        public static void Initialize(IEnumerable<LogHandler> appenders, LogLevel globalLogLevel)
        {
            ...
            foreach (var appender in appenders) AddAppender(appender);

            IsInitialized = true;

            // Disable pre-initialization pooling
            foreach (var logger in _loggers.Values) logger.OnLogEvent -= OnPooledLoggerLog;
        ...
        {
            _appenders += appender;
            foreach (var logger in _loggers.Values) logger.OnLogEvent += appender;
        }
