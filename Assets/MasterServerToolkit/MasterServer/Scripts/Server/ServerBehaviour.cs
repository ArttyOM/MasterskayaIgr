...
namespace MasterServerToolkit.MasterServer
{
    public abstract class ServerBehaviour : MonoBehaviour, IServer
    {
        ...
        [SerializeField] private HelpBox hpInfo = new()
        {
        ...
        [SerializeField]
        [Tooltip(
            "If true, will go through children of this GameObject, and initialize modules that are found on the way")]
        private bool lookInChildrenOnly = true;

        [SerializeField] private List<PermissionEntry> permissions;

        [SerializeField]
        [Range(10, 120)]
        [Tooltip("Frame rate the server will be running. You can limit it from 10 - 120")]
        ...
        [SerializeField] protected float inactivityTimeout = 5f;

        [SerializeField] protected float validationTimeout = 5f;
        ...
        /// </summary>
        private readonly ConcurrentDictionary<ushort, IPacketHandler> handlers = new();

        /// <summary>
        /// Server modules
        /// </summary>
        private readonly Dictionary<Type, IBaseServerModule> modules = new();

        /// <summary>
        /// Initialized server modules list
        /// </summary>
        private readonly HashSet<Type> initializedModules = new();

        /// <summary>
        /// List of connected clients to server
        /// </summary>
        private readonly ConcurrentDictionary<int, IPeer> connectedPeers = new();

        /// <summary>
        /// 
        /// </summary>
        private readonly ConcurrentDictionary<int, IPeer> unauthenticatedPeers = new();
        ...
        public virtual void StartServer(string listenToIp, int listenToPort)
        {
            ...
            var startInfo = new MstProperties();
            ...
            startInfo.Add("\tCertificate Pass",
                string.IsNullOrEmpty(socket.CertificatePath) || !socket.UseSecure ? "Undefined" : "********");
        ...
        private void LookForModules()
        {
            if (lookForModules)
            {
                ...
                if (uninitializedModules.Count > 0)
                    logger.Warn(
                        $"Some of the {GetType().Name.ToSpaceByUppercase()} modules failed to initialize: \n{string.Join(" \n", uninitializedModules.Select(m => m.GetType().ToString()).ToArray())}");
            }
        }
        ...
        protected virtual void OnMessageReceived(IIncomingMessage message)
        {
            try
            {
                ...
                handlers.TryGetValue(message.OpCode, out var handler);

                if (handler == null)
                {
                    logger.Error(
                        $"You are trying to handle message with OpCode [{Extensions.StringExtensions.FromHash(message.OpCode)}]. " +
            ...
            {
                if (Mst.Runtime.IsEditor) throw;

                logger.Error(
                    $"An error occurred while handling a message from client. Message OpCode: [{Extensions.StringExtensions.FromHash(message.OpCode)}], Error: {e}");

                if (!message.IsExpectingResponse) return;
        ...
        public bool InitializeModules()
        {
            ...
            while (true)
            {
                ...
                foreach (var entry in modules)
                {
                    ...
                    if (initializedModules.Contains(entry.Key)) continue;

                    // Not all dependencies have been initialized
                    if (!entry.Value.Dependencies.All(d => initializedModules.Any(d.IsAssignableFrom))) continue;

                    // Not all OPTIONAL dependencies have been initialized
                    if (checkOptional &&
                        !entry.Value.OptionalDependencies.All(d => initializedModules.Any(d.IsAssignableFrom)))
                        continue;
                ...
                if (!changed) return !GetUninitializedModules().Any();
            }
        }
        ...
        {
            connectedPeers.TryGetValue(peerId, out var peer);
            return peer;
        }
        ...
        private void ServerAccessRequestHandler(IIncomingMessage message)
        {
            ...
            Mst.Security.ValidateConnection(accessCheckOptions, (isSuccess, error) =>
            {
                ...
                if (unauthenticatedPeers.TryRemove(message.Peer.Id, out var peer) && peer != null)
                {
