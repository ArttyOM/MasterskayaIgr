...
namespace MasterServerToolkit.MasterServer
{
    public class MstArgs
    {
        ...
        private void ParseArguments()
        {
            ...
            // Android fix
            if (_args == null) _args = Array.Empty<string>();
        ...
        {
            var splitterIndex = input.IndexOf(splitter);

            if (splitterIndex >= 0)
            {
                var key = input.Substring(0, splitterIndex);
                var value = input.Substring(splitterIndex + 1);
        ...
        {
            if (!IsProvided(argName)) return defaultValue;
