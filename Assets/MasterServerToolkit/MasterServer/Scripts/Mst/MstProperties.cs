...
namespace MasterServerToolkit.MasterServer
{
    public class MstProperties
    {
        ...
        public MstProperties(Dictionary<string, string> options)
        {
            ...
            if (options != null) Append(options);
        }
        ...
        {
            if (Has(key)) throw new Exception($"You have already added value with key {key}");
        ...
        {
            if (options == null) return new MstProperties();

            foreach (var key in options.Keys) SetToOptions(key.ToString(), options[key]);
        ...
        public bool AddOrUpdate(IDictionary<string, string> options)
        {
            ...
            {
                if (!differs) differs = Differs(keys[i], options[keys[i]]);
        ...
        {
            return new MstProperties(properties.Where(kvp => kvp.Key.StartsWith(keyFilter))
        ...
        {
            return new MstProperties(properties.Where(kvp => kvp.Value.Contains(valFilter))
                .ToDictionary(kvp => kvp.Key, kvp => kvp.Value));
        }
        ...
        public static MstProperties FromDictionary(IDictionary dictionary)
        {
            ...
            foreach (var key in dictionary.Keys) properties.Set(key.ToString(), dictionary[key].ToString());
        ...
        {
            foreach (var kvp in properties) yield return kvp;
        }
    }
}