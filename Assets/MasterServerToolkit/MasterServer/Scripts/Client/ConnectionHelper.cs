...
namespace MasterServerToolkit.MasterServer
{
    public abstract class ConnectionHelper<T> : SingletonBehaviour<T> where T : MonoBehaviour
    {
        ...
        [SerializeField] protected HelpBox header = new()
        {
        ...
        protected virtual void StartConnectionProcess(string serverIp, int serverPort, int numberOfAttempts)
        {
            ...
            if (!Connection.IsConnected && !Connection.IsConnecting)
            {
                ...
                logger.Info($"{GetType().Name.ToSpaceByUppercase()} is connecting to server at: {serverIp}:{serverPort}"
                    .ToGreen());
            }
            else if (!Connection.IsConnected && Connection.IsConnecting)
            {
                logger.Info(
                    $"{GetType().Name.ToSpaceByUppercase()} is retrying to connect to server at: {serverIp}:{serverPort}. Attempt: {currentAttemptToConnect}"
                        .ToGreen());
            }
            ...
            Connection.WaitForConnection((client) =>
            {
                ...
                if (!client.IsConnected)
                {
                    ...
                    {
                        logger.Info(
                            $"{GetType().Name.ToSpaceByUppercase()} cannot to connect to server at: {serverIp}:{serverPort}"
                                .ToRed());
                        Connection.Close();
        ...
        {
            var totalConnectionTime = Time.realtimeSinceStartup - startConnectionTime;
            logger.Info(
                $"{GetType().Name.ToSpaceByUppercase()} connected to server at: {serverIp}:{serverPort}. Connection time is: {totalConnectionTime}s."
                    .ToGreen());
