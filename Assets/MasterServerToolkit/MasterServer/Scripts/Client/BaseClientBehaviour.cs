...
namespace MasterServerToolkit.MasterServer
{
    public abstract class BaseClientBehaviour : MonoBehaviour, IMstBaseClient
    {
        ...
        [SerializeField] protected bool initModulesAtStart = true;
        ...
        {
            foreach (var module in GetComponentsInChildren<IBaseClientModule>())
            {
        ...
        public virtual void ClearConnection(bool clearHandlers = true)
        {
            if (Connection != null)
            {
                ...
                {
                    foreach (var handler in handlers.Values) Connection.RemoveMessageHandler(handler);
