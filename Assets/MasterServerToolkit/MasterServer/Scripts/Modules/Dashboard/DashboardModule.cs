...
namespace MasterServerToolkit.MasterServer
{
    public class DashboardModule : BaseServerModule
    {
        ...
        /// 
        /// </summary>
        private readonly HashSet<string> sourcesToRemove = new();

        /// <summary>
        /// 
        /// </summary>
        private readonly Dictionary<string, DashboardPeerExtension> registeredSources = new();

        /// <summary>
        /// 
        /// </summary>
        public ConcurrentDictionary<string, SystemInfoPacket> SystemInfo { get; private set; } = new();

        /// <summary>
        /// 
        /// </summary>
        public ConcurrentDictionary<string, ServerInfoPacket> ServerInfo { get; private set; } = new();

        /// <summary>
        /// 
        /// </summary>
        public ConcurrentDictionary<string, List<ModuleInfoPacket>> ModulesInfo { get; private set; } = new();
        ...
        private void SystemInfoMessageHandler(IIncomingMessage message)
        {
            try
            {
                var info = message.AsPacket(new SystemInfoPacket());
        ...
        private void ServerInfoMessageHandler(IIncomingMessage message)
        {
            try
            {
                var info = message.AsPacket(new ServerInfoPacket());
        ...
        private void ModulesInfoMessageHandler(IIncomingMessage message)
        {
            try
            {
                var info = message.AsPacket(new ModuleInfoPacket());
                ...
                var indexOfModule = ModulesInfo[info.Id].FindIndex(x => x.Module == info.Module);
