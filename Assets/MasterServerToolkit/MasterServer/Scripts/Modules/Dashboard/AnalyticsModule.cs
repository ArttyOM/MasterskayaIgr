...
namespace MasterServerToolkit.MasterServer
{
    public class AnalyticsModule : BaseServerModule
    {
        ...
        [Header("Settings")] [SerializeField] private float analyticsSendInterval = 5f;
        ...
        private SystemInfoPacket systemInfoPacket = new();
        ...
        protected override void Awake()
        {
            ...
            dashboardInfoId =
                Mst.Args.AsString(Mst.Args.Names.DashboardInfoId, $"Server_{Mst.Helper.CreateGuidStringN()}");
        }
        ...
        private void ConnectionOpenHandler(IClientSocket client)
        {
            analyticsSendInterval =
                Mathf.Clamp(Mst.Args.AsFloat(Mst.Args.Names.AnalyticsSendInterval, analyticsSendInterval), 2f, 120f);
        ...
        private void SendSystemInfo()
        {
            if (connectionHelper.IsConnected)
                connectionHelper.Connection.SendMessage(MstOpCodes.SystemInfo, systemInfoPacket, (status, respond) =>
                {
                    ...
                    MstTimer.WaitForRealtimeSeconds(0.5f, () => { SendServerInfo(); });
                });
        }
        ...
        private void SendServerInfo()
        {
            if (connectionHelper.IsConnected)
            {
                ...
                connectionHelper.Connection.SendMessage(MstOpCodes.ServerInfo, serverInfoPacket, (status, respond) =>
                {
                    ...
                    MstTimer.WaitForRealtimeSeconds(0.5f, () => { SendModulesInfo(); });
                });
            }
        }
        ...
        private void SendModulesInfo()
        {
            if (connectionHelper.IsConnected)
                ...
                foreach (var module in Server.GetInitializedModules())
                {
                    ...
                    connectionHelper.Connection.SendMessage((ushort) MstOpCodes.ModulesInfo, moduleInfoPacket,
                        (status, respond) =>
                        {
                            if (status != ResponseStatus.Success)
                            {
