...
namespace MasterServerToolkit.MasterServer
{
    ...
    public class ChatModule : BaseServerModule
    {
        ...
        [SerializeField]
        [Tooltip(
            "If the value is true, the chat module will subscribe to the authentication module and automatically add the user to the chat module when logging in. After that, the user is ready to receive chat messages from others.")]
        protected bool useAuthModule = true;

        [SerializeField]
        [Tooltip("If false, chats will be checked through CensorModule to find use of forbidden words in messages")]
        ...
        public override void Initialize(IServer server)
        {
            ...
            if (useAuthModule)
            {
                ...
                {
                    logger.Error(
                        $"{GetType().Name} was set to use {nameof(AuthModule)}, but {nameof(AuthModule)} was not found");
                }
            }

            if (useCensorModule && censorModule == null)
                logger.Error(
                    $"{GetType().Name} was set to use {nameof(CensorModule)}, but {nameof(CensorModule)} was not found");
        }
        ...
        protected virtual void RemoveChatUser(ChatUserPeerExtension user)
        {
            ...
            foreach (var chatChannel in channels) chatChannel.RemoveUser(user);
        ...
        protected virtual ChatChannel GetOrCreateChannel(string channelName, bool ignoreForbidden)
        {
            ...
            if (!ChatChannels.TryGetValue(lowercaseName, out var channel))
            {
                // Check if our new channel name is incorrect
                if (channelName.Length < minChannelNameLength || channelName.Length > maxChannelNameLength) return null;
        ...
        public void ChangeUsername(IPeer peer, string newUsername, bool joinSameChannels = true)
        {
            ...
            if (joinSameChannels)
            {
                foreach (var prevChannel in prevChannels)
                {
                    ...
                    if (channel != null) channel.AddUser(newExtension);
                }
            ...
            try
            {
                ...
                switch (chatMessage.MessageType)
                {
                    case ChatMessageType.ChannelMessage:
                        ...
                        // Find the channel
                        if (!ChatChannels.TryGetValue(chatMessage.Receiver, out var channel) ||
                            !sender.CurrentChannels.Contains(channel))
                        {
                            message.Respond($"You're not in the '{chatMessage.Receiver}' channel",
                    ...
                    case ChatMessageType.PrivateMessage:
                        ...
                        if (!ChatUsers.TryGetValue(chatMessage.Receiver, out var receiver))
                        {
        ...
        protected virtual void OnUserLoggedOutEventHandler(IUserPeerExtension account)
        {
            ...
            if (chatUser != null) RemoveChatUser(chatUser);
        }
        ...
        protected virtual void OnJoinChannelRequestHandler(IIncomingMessage message)
        {
            try
            {
                ...
                if (setFirstChannelAsLocal && chatUser.CurrentChannels.Count == 1) chatUser.DefaultChannel = channel;
        ...
        protected virtual void OnLeaveChannelRequestHandler(IIncomingMessage message)
        {
            try
            {
                ...
                if (!ChatChannels.TryGetValue(channelName, out var channel))
                {
