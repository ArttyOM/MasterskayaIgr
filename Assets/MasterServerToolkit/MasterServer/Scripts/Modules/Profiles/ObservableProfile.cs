...
namespace MasterServerToolkit.MasterServer
{
    ...
    public class ObservableProfile : IEnumerable<IObservableProperty>, IDisposable
    {
        ...
        /// </summary>
        private readonly ConcurrentDictionary<ushort, IObservableProperty> propertiesToBeSent = new();

        /// <summary>
        /// Profile properties list
        /// </summary>
        public ConcurrentDictionary<ushort, IObservableProperty> Properties { get; private set; } = new();

        /// <summary>
        /// Check if profile has changed properties
        /// </summary>
        public bool HasDirtyProperties => propertiesToBeSent.Count > 0;
        ...
        {
            var getResult = Properties.TryGetValue(key, out var val);
            result = val as T;
            return getResult;
        }
        ...
        public void FromStrings(Dictionary<ushort, string> dataData)
        {
            ...
            {
                Properties.TryGetValue(pair.Key, out var property);

                if (property != null) property.Deserialize(pair.Value);
            }
        }
        ...
        {
            foreach (var property in propertiesToBeSent.Values) property.ClearUpdates();
        ...
        public void ReadUpdates(EndianBinaryReader reader)
        {
            ...
            foreach (var updateEntry in dataRead)
                if (Properties.TryGetValue(updateEntry.Key, out var property))
        ...
        public Dictionary<string, string> ToStringsDictionary()
        {
            ...
            foreach (var pair in Properties) dict.Add(pair.Key.ToString(), pair.Value.Serialize());
        ...
        public override string ToString()
        {
            ...
            foreach (var i in ToStringsDictionary()) result.Append($"{i.Key}:{i.Value};");
        ...
        public void FromJson(MstJson json)
        {
            foreach (var property in Properties.Values)
            {
                var key = Extensions.StringExtensions.FromHash(property.Key);

                if (json.HasField(key))
                {
                    var value = json[key];

                    if (!value.IsNull) property.FromJson(value);
                }
            }
        }
