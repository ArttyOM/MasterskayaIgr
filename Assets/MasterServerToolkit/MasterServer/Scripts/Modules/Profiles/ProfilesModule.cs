...
namespace MasterServerToolkit.MasterServer
{
    ...
    public class ProfilesModule : BaseServerModule
    {
        ...
        /// </summary>
        [Tooltip(
            "Time to pass after logging out, until profile will be removed from the lookup. Should be enough for game server to submit last changes")]
        ...
        /// </summary>
        protected readonly HashSet<string> profilesToBeSaved = new();

        /// <summary>
        /// List of profiles that will be sent to clients with updates
        /// </summary>
        protected readonly HashSet<string> profilesToBeSentToClients = new();
        ...
        /// </summary>
        protected readonly ConcurrentDictionary<string, ObservableServerProfile> profilesList = new();
        ...
        {
            get => ignoreProfileMissmatchError;
        ...
        protected async void SendUpdatesToClient(ObservableServerProfile profile, float delay)
        {
            ...
            profile.ClientPeer.SendMessage(MessageHelper.Create(MstOpCodes.UpdateClientProfile, updates),
        ...
        protected async Task UnloadProfile(string userId, float delay)
        {
            ...
            if (profilesList.TryRemove(userId, out var profile) && profile != null)
                profile.OnModifiedInServerEvent -= OnProfileChangedEventHandler;
        }
            ...
            using (var ms = new MemoryStream(data))
            {
                using (var reader = new EndianBinaryReader(EndianBitConverter.Big, ms))
                {
                    ...
                    for (var i = 0; i < count; i++)
                    {
                        ...
                        try
                        {
                            if (profilesList.TryGetValue(userId, out var profile)) profile.ApplyUpdates(updates);
                        }
                        catch (Exception e)
                        {
        ...
        protected virtual async void ClientFillInProfileValuesHandler(IIncomingMessage message)
        {
            ...
            if (!ignoreProfileMissmatchError && clientPropertiesCount != profileExt.Profile.Count)
                logger.Error(string.Format(
                    $"Client requested a profile with {clientPropertiesCount} properties, but server " +
        ...
        protected virtual void GameServerProfileRequestHandler(IIncomingMessage message)
        {
            ...
            profilesList.TryGetValue(userId, out var profile);
