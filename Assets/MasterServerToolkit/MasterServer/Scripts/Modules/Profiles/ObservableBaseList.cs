...
namespace MasterServerToolkit.MasterServer
{
    public abstract class ObservableBaseList<T> : ObservableBase<List<T>>
    {
        ...
        {
            var startIndex = _value.Count > 0 ? _value.Count - 1 : 0;

            _value.AddRange(collection);

            for (var i = startIndex; i < _value.Count; i++)
            {
                var item = _value[i];
                _updates.Enqueue(new ListUpdateEntry()
                {
        ...
        public override byte[] ToBytes()
        {
            ...
            using (var ms = new MemoryStream())
            {
                using (var writer = new EndianBinaryWriter(EndianBitConverter.Big, ms))
                {
                    ...
                    foreach (var item in _value) WriteValue(item, writer);
                }
            ...
            using (var ms = new MemoryStream())
            {
                using (var writer = new EndianBinaryWriter(EndianBitConverter.Big, ms))
                {
                    ...
                    foreach (var update in _updates)
                    {
                        ...
                        if (update.operation != _removeOperation) WriteValue(update.value, writer);
                    }
                }
        ...
        {
            foreach (var v in _value) yield return v;
        }
