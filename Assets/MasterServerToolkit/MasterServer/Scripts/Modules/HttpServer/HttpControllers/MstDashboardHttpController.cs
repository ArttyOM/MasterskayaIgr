...
namespace MasterServerToolkit.MasterServer
{
    public class MstDashboardHttpController : HttpController
    {
        ...
        [Header("Modules")] [SerializeField] private DashboardModule dashboardModule;

        [Header("Application")] [SerializeField]
        private TextAsset appHtmlFile;

        [SerializeField] private TextAsset appVueFile;

        [Header("System")] [SerializeField] private MstDashboardComponent[] appComponents;

        [Header("Mst")] [SerializeField] private Texture2D logoImage;
        ...
        private readonly Dictionary<string, string> components = new();
        ...
        private void Start()
        {
            ...
            foreach (var appComponent in appComponents)
                ...
                {
                    var htmlTemplate = appComponent.htmlTemplate.text;
                    var vueScript = appComponent.vueScript.text;
                    Inject(htmlTemplate, vueScript, VUE_COMPONENT_TEMPLATE, out var result);
        ...
        private async void OnDashboardRequestHandlerAsync(HttpListenerRequest request, HttpListenerResponse response)
        {
            try
            {
                foreach (var component in components) Inject(component.Value, appVue, component.Key, out appVue);
                ...
                var logoBase64 = $"data:image/png;base64,{Convert.ToBase64String(logoRawData)}";
                Inject(logoBase64, appHtml, MST_LOGO, out appHtml);

                var contents = Encoding.UTF8.GetBytes(appHtml);
        ...
        private async void OnGetServerInfoRequestHandlerAsync(HttpListenerRequest request,
            HttpListenerResponse response)
        {
            try
            {
                ...
                foreach (var kvp in dashboardModule.ServerInfo) json.AddField(kvp.Key, kvp.Value.ToJson());

                var contents = Encoding.UTF8.GetBytes(json.ToString());
        ...
        private async void OnGetSystemInfoRequestHandlerAsync(HttpListenerRequest request,
            HttpListenerResponse response)
        {
            try
            {
                ...
                foreach (var kvp in dashboardModule.SystemInfo) json.AddField(kvp.Key, kvp.Value.ToJson());
        ...
        private async void OnGetModulesInfoRequestHandlerAsync(HttpListenerRequest request,
            HttpListenerResponse response)
        {
            try
            {
                ...
                {
                    var modules = new MstJson();

                    foreach (var item in kvp.Value) modules.Add(item.Data);
