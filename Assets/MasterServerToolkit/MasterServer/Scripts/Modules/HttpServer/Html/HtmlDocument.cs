...
namespace MasterServerToolkit.MasterServer.Web
{
    public class HtmlDocument
    {
        ...
        public void AddMeta(params KeyValuePair<string, string>[] attributes)
        {
            ...
            foreach (var kvp in attributes) meta.SetAttribute(kvp.Key, kvp.Value);
        ...
        public XmlElement CreateElement(string name, bool isBlock = true)
        {
            ...
            if (isBlock) el.InnerText = string.Empty;
        ...
        {
            // Append metas
            foreach (var meta in metas) Head.AppendChild(meta);

            // Append links
            foreach (var link in Links) Head.AppendChild(link.ToNode(document));

            // Append title
            Head.AppendChild(title);

            foreach (var script in Scripts) Body.AppendChild(script.ToNode(document));
