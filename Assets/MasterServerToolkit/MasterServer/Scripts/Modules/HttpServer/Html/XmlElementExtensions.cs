...
namespace MasterServerToolkit.MasterServer.Web
{
    public static class XmlElementExtensions
    {
        ...
        {
            var newClasses = className.Split(new char[] {' '}, StringSplitOptions.RemoveEmptyEntries).ToList();
        ...
        public static void RemoveClass(this XmlElement el, string className)
        {
            if (el.HasAttribute("class"))
            {
                var classes = el.GetAttribute("class").Split(new char[] {' '}, StringSplitOptions.RemoveEmptyEntries)
                    .ToList();

                if (!classes.ToList().Contains(className.Trim())) classes.Remove(className);
