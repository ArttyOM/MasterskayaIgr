...
namespace MasterServerToolkit.MasterServer
{
    ...
    {
        private Queue<string> messageQueueData = new();
        private Dictionary<string, WsControllerMessageHandler> messageHandlers = new();
        ...
        {
            if (messageQueueData.Count <= 0) return;
            ...
            lock (messageQueueData)
            {
                while (messageQueueData.Count > 0)
                    try
                    {
                        ...
                        var messageData = new MstJson(messageQueueData.Dequeue());
