...
namespace MasterServerToolkit.MasterServer
{
    ...
    public class HttpServerModule : BaseServerModule
    {
        ...
        /// </summary>
        private readonly ConcurrentDictionary<string, OnHttpRequestDelegate> httpRequestActions = new();

        /// <summary>
        /// List of surface controllers
        /// </summary>
        public Dictionary<Type, IHttpController> Controllers { get; protected set; } = new();
        ...
        protected override void Awake()
        {
            ...
            heartBeatCheckInterval =
                Mathf.Clamp(Mst.Args.AsFloat(Mst.Args.Names.WebServerHeartbeatCheckInterval, heartBeatCheckInterval),
                    2f, 120f);
            checkHeartBeatUrl = Mst.Args.AsString($"live-{Mst.Args.Names.WebServerHeartbeatCheckPage}",
                $"live-{Mst.Helper.CreateGuidString()}");
        ...
        private void ValidateRequest(HttpListenerContext context, Action successCallback)
        {
            if (useCredentials && context.User != null)
            {
                ...
                var clientUsername = identity.Name;
                var clientPassword = identity.Password;
                ...
                {
                    var contents = Encoding.UTF8.GetBytes(Default401Page());
        ...
        private async void CheckHeartBeat()
        {
            ...
            try
            {
                IPAddress.TryParse(httpAddress, out var address);
                var url = CreateHttpRequestPrefix(checkHeartBeatUrl,
                    address != null ? address.ToString() : "127.0.0.1");

                if (url.EndsWith("/"))
                    url = url.Substring(0, url.Length - 1);

                var hbWebRequest = (HttpWebRequest) WebRequest.Create(url);
        ...
        private void RegisterDefaultControllers()
        {
            ...
            {
                var contents = Encoding.UTF8.GetBytes(DefaultIndexPageHtml());
            ...
            {
                var contents = Encoding.UTF8.GetBytes("Ok");
        ...
        protected virtual string Default401Page()
        {
            ...
            html.AddMeta(new KeyValuePair<string, string>("name", "viewport"),
                new KeyValuePair<string, string>("content", "width=device-width, initial-scale=1"));
            html.AddMeta(new KeyValuePair<string, string>("name", "description"),
            ...
            html.AddMeta(new KeyValuePair<string, string>("name", "author"),
                new KeyValuePair<string, string>("content", "Master Server Toolkit"));
            ...
            p1.InnerText =
                $"This is default 401 page. You can override it by overloading {nameof(Default401Page)} method in {nameof(HttpServerModule)}";
            col.AppendChild(p1);
        ...
        protected virtual string Default404Page()
        {
            ...
            var p1 = html.CreateElement("p");
            p1.InnerText =
                $"This is default 404 page. You can override it by overloading {nameof(Default404Page)} method in {nameof(HttpServerModule)}";
            col.AppendChild(p1);
        ...
        protected virtual string DefaultIndexPageHtml()
        {
            ...
            var p1 = html.CreateElement("p");
            p1.InnerText =
                $"This is default Index page. You can override it by overloading {nameof(DefaultIndexPageHtml)} method in {nameof(HttpServerModule)}";
            col.AppendChild(p1);
        ...
        protected virtual void HttpGetRequestHandler(HttpListenerRequest request, HttpListenerResponse response)
        {
            ...
            {
                // Let's parse user
                var cleanRawUrl = ClearRawUrl(request);

                // Let's ceate url key
                var urlKey = CreateHttpRequestHandlerKey(cleanRawUrl, HttpMethod.GET);
                ...
                {
                    var contents = Encoding.UTF8.GetBytes(Default404Page());
            ...
            {
                var contents = Encoding.UTF8.GetBytes(e.ToString());
        ...
        protected virtual void HttpPostRequestHandler(HttpListenerRequest request, HttpListenerResponse response)
        {
            ...
            // Let's ceate url key
            var urlKey = CreateHttpRequestHandlerKey(cleanRawUrl, HttpMethod.POST);
            ...
            {
                var contents = Encoding.UTF8.GetBytes("There is no such controller");
        ...
        protected virtual void HttpPutRequestHandler(HttpListenerRequest request, HttpListenerResponse response)
        {
            ...
            // Let's ceate url key
            var urlKey = CreateHttpRequestHandlerKey(cleanRawUrl, HttpMethod.PUT);
        ...
        public string CreateHttpRequestPrefix(string path, string httpAddress, int httpPort)
        {
            ...
            var scheme = UseSecure ? "https://" : "http://";
            var url = $"{httpAddress}:{httpPort}/{path}/".Replace("//", "/");
        ...
        {
            string url;
            var prefix = CreateHttpRequestPrefix(path);
            ...
            if (httpRequestActions.ContainsKey(url)) throw new Exception($"Handler [{url}] already exists");
