    ...
    public class WorldRoomsModule : RoomsModule
    {
        ...
        public override void Initialize(IServer server)
        {
            ...
            if (spawnersModule)
                ...
                logger.Error(
                    $"{GetType().Name} was set to use {nameof(SpawnersModule)}, but {nameof(SpawnersModule)} was not found");
        ...
        private async void Spawners_OnSpawnerRegisteredEvent(RegisteredSpawner spawner)
        {
            ...
            foreach (var zoneScene in zoneScenes)
        ...
        private void GetZoneRoomInfoMessageHandler(IIncomingMessage message)
        {
            ...
            var zoneId = message.AsString();

            var zoneRoom = roomsList.Values
            ...
            var players = zoneRoom.Players.Values.Where(pl => pl.HasExtension<IUserPeerExtension>())
