...
namespace MasterServerToolkit.MasterServer
{
    public class BaseLobby : ILobby
    {
        ...
        /// </summary>
        public int PlayerCount => membersByUsernameList.Count;
        ...
        protected LobbyMember GameMaster
        {
            ...
            set
            {
                if (!Config.EnableGameMasters) return;
        ...
        public bool AddPlayer(LobbyUserPeerExtension lobbyUser, out string error)
        {
            ...
            if (GameMaster == null) PickNewGameMaster(false);
        ...
        public void RemovePlayer(LobbyUserPeerExtension lobbyUser)
        {
            ...
            membersByUsernameList.TryGetValue(username, out var member);
            ...
            if (lobbyUser.CurrentLobby == this) lobbyUser.CurrentLobby = null;

            // Remove member from it's current team
            if (member.Team != null) member.Team.RemoveMember(member);

            // Change the game master
            if (GameMaster == member) PickNewGameMaster();
        ...
        {
            membersByPeerIdList.TryGetValue(playerExt.Peer.Id, out var member);
            return member;
        }
        ...
        public bool SetPlayerProperty(LobbyMember member, string key, string value)
        {
            ...
            if (!IsPlayerPropertyChangeable(member, key, value)) return false;
        ...
        {
            if (!Config.AllowPlayersChangeLobbyProperties) return false;

            if (Config.EnableGameMasters)
            {
                membersByPeerIdList.TryGetValue(setter.Peer.Id, out var member);

                if (GameMaster != member) return false;
            }
        ...
        {
            if (!membersByUsernameList.ContainsKey(member.Username)) return;

            member.IsReady = state;

            OnPlayerReadyStatusChange(member);

            if (membersByUsernameList.Values.All(m => m.IsReady)) OnAllPlayersReady();
        }
        ...
        public bool TryJoinTeam(string teamName, LobbyMember member)
        {
            ...
            if (!newTeam.AddMember(member)) return false;
        ...
        public virtual bool StartGame()
        {
            ...
            var region = propertiesList.AsString(Mst.Args.Names.RoomRegion);
        ...
        {
            if (task == null || gameSpawnTask == task) return;
        ...
        public void Destroy()
        {
            ...
            // Remove players
            foreach (var member in membersByUsernameList.Values.ToList()) RemovePlayer(member.Extension);
        ...
        {
            if (gameSpawnTask.FinalizationPacket == null) return;
        ...
        {
            foreach (var peer in subscribersList) peer.SendMessage(message, DeliveryMethod.Reliable);
        }
        ...
        protected virtual void OnPlayerAdded(LobbyMember member)
        {
            ...
            var msg = MessageHelper.Create((ushort) MstOpCodes.LobbyMemberJoined,
                member.GenerateDataPacket().ToBytes());
        ...
        protected virtual void OnLobbyStateChange(LobbyState state)
        {
            ...
            foreach (var lobbyMember in membersByUsernameList.Values) SetReadyState(lobbyMember, false);
        ...
        {
            if (!Config.StartGameWhenAllReady) return;

            if (teamsList.Values.Any(t => t.PlayersCount < t.MinPlayers)) return;
