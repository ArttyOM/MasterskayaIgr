...
namespace MasterServerToolkit.MasterServer
{
    public class NotificationModule : BaseServerModule
    {
        ...
        [SerializeField]
        [Tooltip(
            "If true, notification module will subscribe to rooms module to be able to send notifications to room players")]
        ...
        /// List of recipients
        /// </summary>
        protected ConcurrentDictionary<string, NotificationRecipient> registeredRecipients = new();

        /// <summary>
        /// List of messages to be sent to newly logged in users
        /// </summary>
        protected List<string> promisedMessages = new();
        ...
        public override void Initialize(IServer server)
        {
            ...
            if (useRoomsModule && !roomsModule)
                logger.Error(
                    $"{GetType().Name} was set to use {nameof(RoomsModule)}, but {nameof(RoomsModule)} was not found. It means that room players cannot receive notifications");

            server.RegisterMessageHandler(MstOpCodes.SubscribeToNotifications,
                OnSubscribeToNotificationsMessageHandler);
            server.RegisterMessageHandler(MstOpCodes.UnsubscribeFromNotifications,
                OnUnsubscribeFromNotificationsMessageHandler);
        ...
        public override MstJson JsonInfo()
        {
            ...
            try
            {
                json.AddField("description",
                    $"This is the {nameof(NotificationModule)} theat helps rooms send notifications to their players or helps admins of Master server send notifications to list of recipients");
        ...
        {
            registeredRecipients.TryGetValue(userId, out var r);
            return r;
        }
        ...
        public virtual void NoticeToAll(string textMessage, bool addToPromise = false)
        {
            ...
            foreach (var recipient in registeredRecipients.Values) recipient.Notify(textMessage);

            if (addToPromise && !promisedMessages.Contains(textMessage))
            {
                if (promisedMessages.Count >= maxPromisedMessages) promisedMessages.RemoveAt(0);
