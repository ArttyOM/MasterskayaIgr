...
namespace MasterServerToolkit.MasterServer
{
    public class RegisteredSpawner
    {
        ...
        {
            // Unlimited
            if (Options.MaxProcesses == 0) return true;

            // Spawner is busy
            if (_queue.Count + ProcessesRunning >= Options.MaxProcesses) return false;
        ...
        public void UpdateQueue()
        {
            ...
            if (_beingSpawned.Count >= MaxConcurrentRequests)
            {
                ...
                foreach (var finishedSpawn in finishedSpawns) _beingSpawned.Remove(finishedSpawn);
            }

            // If we're still at the maximum concurrent requests
            if (_beingSpawned.Count >= MaxConcurrentRequests) return;
        ...
        public void SendKillRequest(int spawnId, KillRequestCallback callback)
        {
            ...
            Peer.SendMessage((ushort) MstOpCodes.KillProcessRequest, packet,
                (status, response) => { callback.Invoke(status == ResponseStatus.Success); });
        }
