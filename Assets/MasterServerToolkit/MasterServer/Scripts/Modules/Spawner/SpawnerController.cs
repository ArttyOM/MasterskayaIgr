...
namespace MasterServerToolkit.MasterServer
{
    public class SpawnerController : ISpawnerController
    {
        ...
        /// </summary>
        protected static object processLock = new();

        /// <summary>
        /// List of spawned processes
        /// </summary>
        protected Dictionary<int, Process> processes = new();
        ...
        private static void SpawnProcessRequestHandler(IIncomingMessage message)
        {
            try
            {
                ...
                var controller = Mst.Server.Spawners.GetSpawnerController(data.SpawnerId);
        ...
        {
            Logger.Info(
                $"Spawn handler started handling a request to spawn process for spawn controller [{SpawnerId}]");
            ...
            var machinePortArgument = Mst.Server.Spawners.GetAvailablePort();
            ...
            try
            {
                new Thread(() =>
                {
                    try
                    {
                        ...
                        {
                            Logger.Info(
                                $"Process [{startProcessInfo.FileName}] started. Spawn Id: {data.SpawnTaskId}, pid: {process.Id}");
                            processStarted = true;
                    ...
                    {
                        if (!processStarted) message.Respond(ResponseStatus.Failed);
        ...
        {
            Logger.Info(
                $"Kill request handler started handling a request to kill a process with id [{spawnId}] for spawn controller with id [{SpawnerId}]");
        ...
        public void KillProcesses()
        {
            ...
            lock (processLock)
            {
                foreach (var process in processes.Values) list.Add(process);
            }

            foreach (var process in list) process.Kill();
        }
