...
namespace MasterServerToolkit.MasterServer
{
    public class SpawnersModule : BaseServerModule
    {
        ...
        protected readonly ConcurrentDictionary<int, RegisteredSpawner> spawnersList = new();
        protected readonly ConcurrentDictionary<int, SpawnTask> spawnTasksList = new();
        ...
        public override void Initialize(IServer server)
        {
            ...
            server.RegisterMessageHandler(MstOpCodes.UpdateSpawnerProcessesCount,
                SetSpawnedProcessesCountRequestHandler);
        ...
        public virtual RegisteredSpawner CreateSpawner(IPeer peer, SpawnerOptions options)
        {
            ...
            var peerSpawners =
                peer.GetProperty(MstPeerPropertyCodes.RegisteredSpawners) as Dictionary<int, RegisteredSpawner>;
        ...
        public virtual SpawnTask Spawn(MstProperties options, string region)
        {
            ...
            {
                logger.Warn(
                    $"No spawner was returned after filtering. Region: {options.AsString(Mst.Args.Names.RoomRegion, string.IsNullOrEmpty(region) ? "International" : region)}");
                return null;
            }
            ...
            if (availableSpawner == null) return null;
        ...
        public virtual List<RegisteredSpawner> GetSpawners(string region)
        {
            ...
            if (string.IsNullOrEmpty(region)) return spawnersList.Values.ToList();
        ...
        protected virtual void ClientsSpawnRequestHandler(IIncomingMessage message)
        {
            ...
            var prevRequest = peer.GetProperty(MstPeerPropertyCodes.ClientSpawnRequest) as SpawnTask;
            ...
            task.OnStatusChangedEvent += (status) =>
            {
                ...
                if (task.Requester != null && task.Requester.IsConnected) peer.SendMessage(msg);
            };
        ...
        {
            var prevRequest = message.Peer.GetProperty(MstPeerPropertyCodes.ClientSpawnRequest) as SpawnTask;
        ...
        protected virtual void GetCompletionDataRequestHandler(IIncomingMessage message)
        {
            ...
            if (!spawnTasksList.TryGetValue(spawnId, out var task))
            {
        ...
        protected virtual void RegisterSpawnerRequestHandler(IIncomingMessage message)
        {
            ...
            logger.Debug(
                $"Client [{message.Peer.Id}] was successfully registered as spawner [{spawner.SpawnerId}] with options: {options}");
        ...
        protected virtual void RegisterSpawnedProcessRequestHandler(IIncomingMessage message)
        {
            ...
            if (!spawnTasksList.TryGetValue(data.SpawnId, out var task))
            {
        ...
        protected virtual void CompleteSpawnProcessRequestHandler(IIncomingMessage message)
        {
            ...
            if (spawnTasksList.TryGetValue(data.SpawnTaskId, out var task))
            {
        ...
        private void SetSpawnedProcessesCountRequestHandler(IIncomingMessage message)
        {
            ...
            if (spawnersList.TryGetValue(packet.A, out var spawner)) spawner.UpdateProcessesCount(packet.B);
        }
