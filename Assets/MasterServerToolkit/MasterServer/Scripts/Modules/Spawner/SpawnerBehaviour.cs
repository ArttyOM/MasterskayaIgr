...
namespace MasterServerToolkit.MasterServer
{
    public class SpawnerBehaviour : MonoBehaviour
    {
        ...
        [SerializeField] private HelpBox headerEditor = new()
        {
        ...
        [SerializeField] private HelpBox headerWarn = new()
        {
            Text =
                $"It will start ONLY if '-msfStartSpawner' argument is found, or if StartSpawner() is called manually from your scripts",
        ...
        [SerializeField]
        [Tooltip(
            "Use this to set whether or not to spawn room/server for browser games. This feature works only if game server uses websocket transport for connections")]
        protected bool spawnWebSocketServers = false;

        [SerializeField]
        [Tooltip("Spawner region used when you are trying to start rooms by given region. Empty means International")]
        protected string region = "";

        [Header("Runtime Settings")]
        [SerializeField]
        [Tooltip("If true, kills all spawned processes when spawners stopped")]
        protected bool killProcessesWhenStop = true;

        [Header("Editor Settings")] [SerializeField]
        private HelpBox hpEditor = new()
        {
        ...
        [SerializeField]
        [Tooltip(
            "If true, when running in editor, spawner server will start automatically (after connecting to master)")]
        protected bool autoStartInEditor = true;

        [SerializeField]
        [Tooltip(
            "If true, and if running in editor, path to executable will be overriden, and a value from 'exePathFromEditor' will be used.")]
        ...
        protected virtual void OnConnectedToMasterEventHandler(IClientSocket client)
        {
            ...
            if (Mst.Args.StartSpawner || autoStartInEditor && Mst.Runtime.IsEditor) StartSpawner();
        }
        ...
        public virtual void StartSpawner()
        {
            ...
            Mst.Server.Spawners.RegisterSpawner(spawnerOptions, (controller, error) =>
            {
                ...
                spawnerController.SpawnSettings.UseWebSockets =
                    Mst.Args.AsBool(Mst.Args.Names.UseWebSockets, spawnWebSocketServers);
