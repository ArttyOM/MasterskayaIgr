...
namespace MasterServerToolkit.MasterServer
{
    ...
    public class AuthModule : BaseServerModule
    {
        ...
        [Header("Settings")]
        [SerializeField]
        [Tooltip("Min number of username characters. Will not be used in guest username")]
        ...
        [Header("Guest Settings")] [SerializeField] [Tooltip("If true, players will be able to log in as guests")]
        ...
        [SerializeField]
        [Tooltip("How many days token will be valid before expire. The token will also be updated each login")]
        ...
        [Header("Security")]
        [SerializeField]
        [Tooltip("Secret code to create user auth token. Change it for your own project")]
        ...
        /// </summary>
        protected ConcurrentDictionary<string, IUserPeerExtension> loggedInUsers { get; set; } = new();
        ...
        {
            var prefix = string.IsNullOrEmpty(guestPrefix) ? "user#" : guestPrefix;
        ...
        {
            if (loggedInUsers.TryGetValue(id, out var user))
                return user;
            else
                return default;
        }
        ...
        {
            var list = new List<IUserPeerExtension>();

            foreach (var id in ids)
                if (TryGetLoggedInUserById(id, out var user))
                    if (user != null)
                        list.Add(user);
        ...
        protected virtual async void GetPasswordResetCodeMessageHandler(IIncomingMessage message)
        {
            ...
            var sentResult = await mailer.SendMailAsync(userAccount.Email, "Password Reset Code", emailBody.ToString());
        ...
        protected virtual async void GetEmailConfirmationCodeMessageHandler(IIncomingMessage message)
        {
            ...
            await authDatabaseAccessor.SaveEmailConfirmationCodeAsync(userExtension.Account.Email,
            ...
            var sentResult = await mailer.SendMailAsync(userExtension.Account.Email, "E-mail confirmation",
                emailBody.ToString());
        ...
        protected virtual async void SignUpMessageHandler(IIncomingMessage message)
        {
            try
            {
                ...
                var isLoggedIn = userExtension != null;
                ...
                var hasUsername = !userCredentials.IsValueEmpty(MstDictKeys.USER_NAME);
                var hasPassword = !userCredentials.IsValueEmpty(MstDictKeys.USER_PASSWORD);
                var hasEmail = !userCredentials.IsValueEmpty(MstDictKeys.USER_EMAIL);
                ...
                {
                    logger.Error(
                        $"Invalid usernanme length. Min length is {usernameMinChars} and max length is {usernameMaxChars}");
                    message.Respond(ResponseStatus.Invalid);
                    return;
                }
        ...
        protected virtual async void SignInMessageHandler(IIncomingMessage message)
        {
            try
            {
                ...
                var userAccount = await RunAuthFactory(message.Peer, userCredentials, message);
            ...
            IIncomingMessage message)
        {
            ...
            // Create new password
            var newPassword = Mst.Helper.CreateRandomAlphanumericString(userPasswordMinChars);
            ...
            if (mailer == null)
            {
                message.Respond("Couldn't send creadentials to your e-mail. Please contact support",
            ...
            var sentResult = await mailer.SendMailAsync(userEmail, "Login by Email", emailBody.ToString());
        ...
        {
            var dateTime = DateTime.UtcNow.AddDays(tokenExpiresInDays).ToString();
            var token = $"{userAccount.Id}/{userAccount.Username}/{dateTime}";
        ...
        protected virtual bool IsTokenExpired(string token)
        {
            ...
            var decryptedToken = Mst.Security.DecryptStringAES(token, tokenSecret);
            var chars = new char[] {'/'};
            var values = decryptedToken.Split(chars, StringSplitOptions.RemoveEmptyEntries);

            if (values.Length == 3 && DateTime.TryParse(values[2], out var expireDate))
        ...
        /// <param name="userCredentials"></param>
        /// <returns></returns>
        protected virtual async Task<IAccountInfoData> SignInWithLoginAndPassword(IPeer peer,
            MstProperties userCredentials, IIncomingMessage message)
        {
            // Trying to get user extension from peer
            var userPeerExtension = peer.GetExtension<IUserPeerExtension>();
            ...
            IIncomingMessage message)
        {
            ...
            // Get token
            var token = userCredentials.AsString(MstDictKeys.USER_AUTH_TOKEN);
            ...
            var userAccount = await authDatabaseAccessor.GetAccountByTokenAsync(token);
            ...
            IIncomingMessage message)
        {
            ...
            var anotherGuestOnTheSameDevice = userAccount != null && IsUserLoggedInById(userAccount.Id);
