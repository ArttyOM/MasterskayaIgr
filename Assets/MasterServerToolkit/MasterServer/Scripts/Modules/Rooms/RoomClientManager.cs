...
namespace MasterServerToolkit.MasterServer
{
    public class RoomClientManager : RoomClient<RoomClientManager>
    {
        ...
        [Header("Settings")]
        [SerializeField]
        [Tooltip("The name of the room that will be loaded after the client leaves the room")]
        ...
        {
            roomConnection.SendMessage(MstOpCodes.ValidateRoomAccessRequest, Mst.Client.Rooms.ReceivedAccess.Token,
                (status, response) =>
                {
                    ...
                    else
                    {
                        ...
                        Mst.Events.Invoke(MstEventKeys.showOkDialogBox,
                            new OkDialogBoxEventMessage(response.AsString("Error"), null));
                    }
                });
        }
        ...
        {
            ScenesLoader.LoadSceneByName(Mst.Client.Rooms.ReceivedAccess.SceneName,
                (progressValue) =>
                {
                    Mst.Events.Invoke(MstEventKeys.showLoadingInfo,
                        $"Loading scene {Mathf.RoundToInt(progressValue * 100f)}% ... Please wait!");
                }, null);
        }
        ...
        protected virtual void LoadOfflineScene()
        {
            if (!string.IsNullOrEmpty(offlineRoomScene))
                ScenesLoader.LoadSceneByName(offlineRoomScene,
                    (progressValue) =>
                    {
                        Mst.Events.Invoke(MstEventKeys.showLoadingInfo,
                            $"Loading scene {Mathf.RoundToInt(progressValue * 100f)}% ... Please wait!");
