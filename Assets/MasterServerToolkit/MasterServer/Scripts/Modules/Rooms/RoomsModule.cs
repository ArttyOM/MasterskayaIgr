...
namespace MasterServerToolkit.MasterServer
{
    public class RoomsModule : BaseServerModule, IGamesProvider
    {
        ...
        [Header("Permissions")] [Tooltip("Minimal permission level, necessary to register a room")] [SerializeField]
        ...
        /// </summary>
        protected readonly ConcurrentDictionary<int, RegisteredRoom> roomsList = new();
        ...
        {
            foreach (var registeredRoom in roomsList.Values) registeredRoom.ClearTimedOutAccesses();
        }
        ...
        private void OnRegisteredPeerDisconnect(IPeer peer)
        {
            ...
            if (peerRooms == null) return;

            logger.Debug(
                $"Client {peer.Id} was disconnected from server and it has registered rooms that also must be destroyed");

            // Create a copy so that we can iterate safely
            var registeredRooms = peerRooms.Values.ToList();

            foreach (var registeredRoom in registeredRooms) DestroyRoom(registeredRoom);
        }
        ...
        public virtual void DestroyRoom(RegisteredRoom room)
        {
            ...
            {
                var peerRooms =
                    peer.GetProperty(MstPeerPropertyCodes.RegisteredRooms) as Dictionary<int, RegisteredRoom>;

                // Remove the room from peer
                if (peerRooms != null) peerRooms.Remove(room.RoomId);
            }
        ...
        public virtual IEnumerable<GameInfoPacket> GetPublicGames(IPeer peer, MstProperties filters)
        {
            var rooms = filters != null && filters.Has(MstDictKeys.ROOM_ID)
                ? roomsList.Values.Where(r =>
                    (r.Players.ContainsKey(peer.Id) || r.Options.IsPublic) &&
                    r.RoomId == filters.AsInt(MstDictKeys.ROOM_ID))
            ...
            foreach (var room in rooms)
            {
                ...
                var players = room.Players.Values.Where(pl => pl.HasExtension<IUserPeerExtension>())
        ...
        {
            roomsList.TryGetValue(roomId, out var r);
            return r;
        }
        ...
        protected virtual void ValidateRoomAccessRequestHandler(IIncomingMessage message)
        {
            ...
            if (!TryGetRoom(data.RoomId, out var room))
            {
                message.Respond("Room does not exist", ResponseStatus.Failed);
                return;
            }
            ...
            if (userExt != null) packet.Username = userExt.Username ?? "";
