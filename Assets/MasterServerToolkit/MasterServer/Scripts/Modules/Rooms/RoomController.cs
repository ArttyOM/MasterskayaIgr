...
namespace MasterServerToolkit.MasterServer
{
    ...
    public delegate void RoomAccessProviderDelegate(RoomAccessProviderCheck accessCheckOptions,
    ...
    public class RoomController
    {
        ...
        {
            get => accessProvider ?? DefaultAccessProvider;
            set => accessProvider = value;
        }
        ...
        public void NotifyPlayerLeft(int peerId)
        {
            ...
            {
                if (!successful) Logger.Error(error);
        ...
        /// <param name="callback"></param>
        public void DefaultAccessProvider(RoomAccessProviderCheck accessCheckOptions,
            RoomAccessProviderCallbackDelegate callback)
        {
        ...
        {
            Options.IsPublic = true;
            SaveOptions(Options, (successful, error) => { callback.Invoke(); });
        }
        ...
        private void ProvideRoomAccessCheckHandler(IIncomingMessage message)
        {
            ...
            {
                message.Respond($"There's no room controller with room id {provideRoomAccessCheckPacket.RoomId}",
            ...
            roomController.AccessProvider.Invoke(roomAccessProviderCheck, (access, error) =>
            {
                ...
                if (Logger.IsLogging(LogLevel.Trace))
                    Logger.Trace("Room controller gave address to peer " + provideRoomAccessCheckPacket.PeerId + ":" +
                                 access);
            });
            ...
            MstTimer.WaitForRealtimeSeconds(Mst.Server.Rooms.AccessProviderTimeout, () =>
            {
                if (!isProviderDone)
                {
                    ...
                    Logger.Error(
                        $"Access provider took longer than {Mst.Server.Rooms.AccessProviderTimeout} seconds to provide access. " +
