...
namespace MasterServerToolkit.MasterServer
{
    public abstract class RoomClient<T> : SingletonBehaviour<T> where T : MonoBehaviour
    {
        ...
        private HelpBox editorHelp = new()
        {
        ...
        [SerializeField]
        [Tooltip(
            "If signInAsGuest is not true system will try sign in as registereg user in test mode using this username")]
        ...
        [SerializeField]
        [Tooltip(
            "If signInAsGuest is not true system will try sign in as registereg user in test mode using this password")]
        ...
        protected override void Awake()
        {
            ...
            Mst.Events.AddListener(MstEventKeys.leaveRoom, (message) => { Disconnect(); });

            Mst.Events.AddListener(MstEventKeys.goToZone, (message) => { isChangingZone = message.AsBool(); });
        ...
        protected virtual void Start()
        {
            if (Mst.Runtime.IsEditor && autoStartInEditor)
                MstTimer.WaitForSeconds(autoStartDelay, () => { AutostartInEditor(); });
        }
            ...
            Mst.Client.Matchmaker.FindGames((games) =>
            {
                if (games.Count == 0)
                {
                    ...
                    Mst.Events.Invoke(MstEventKeys.showOkDialogBox,
                        new OkDialogBoxEventMessage("No games found", null));
                    Mst.Events.Invoke(MstEventKeys.hideLoadingInfo);
                    return;
                }
                ...
                Mst.Client.Rooms.GetAccess(games.First().Id, (access, getAccessError) =>
                {
                    ...
                    if (!string.IsNullOrEmpty(getAccessError))
                    {
                        ...
                        Mst.Events.Invoke(MstEventKeys.showOkDialogBox,
                            new OkDialogBoxEventMessage(getAccessError, null));
                        Disconnect();
                    }
                });
            });
