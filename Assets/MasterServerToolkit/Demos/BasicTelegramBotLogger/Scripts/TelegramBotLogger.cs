...
namespace MasterServerToolkit.Examples.BasicTelegramBotLogger
{
    ...
    {
        [Header("Settings")] [SerializeField] private string botApiToken;
        [SerializeField] private string[] chatIds;
        ...
        {
            var logString = $"{condition} \n {stackTrace}";

            switch (type)
            {
                case LogType.Error:
                case LogType.Exception:
                    foreach (var userId in chatIds) SendMessageToChat(userId, logString);
                    break;
            }
        }
