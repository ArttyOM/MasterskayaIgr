...
namespace MasterServerToolkit.Examples.BasicProfile
{
    public class InventoryView : UIView
    {
        ...
        [Header("Inventory")] [SerializeField] private ItemUI storeItemUIPrefab;
        [SerializeField] private RectTransform storeItemsContainer;
        [SerializeField] private ItemUI backpackItemUIPrefab;
        [SerializeField] private RectTransform backpackItemsContainer;
        [SerializeField] private StoreOffersDatabase storeOffers;

        [Header("Currencies")] [SerializeField]
        private UIProperty bronzeUIProperty;

        [SerializeField] private UIProperty silverUIProperty;
        [SerializeField] private UIProperty goldUIProperty;
        ...
        {
            if (itemUis.TryGetValue(key, out var itemUI)) itemUI.gameObject.SetActive(false);
        }
        ...
        private void DrawStoreOffers()
        {
            ...
            foreach (var storeOffer in storeOffers.Offers)
            {
                ...
                storeOfferUIInstance.OnClick(() => { BuyItem(storeOffer); });
            }
        }
                ...
                Mst.Client.Connection.SendMessage(MessageOpCodes.BuyDemoItem, data, (status, responce) =>
                {
                    if (status != Networking.ResponseStatus.Success)
                    {
                        Mst.Events.Invoke(MstEventKeys.showOkDialogBox,
                            new OkDialogBoxEventMessage($"An error occurred: {responce.AsString("Unhandled")}", null));
                        return;
                    }
