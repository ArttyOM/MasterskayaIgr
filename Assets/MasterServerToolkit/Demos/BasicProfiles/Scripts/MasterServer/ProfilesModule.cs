...
namespace MasterServerToolkit.Examples.BasicProfile
{
    public class ProfilesModule : MasterServer.ProfilesModule
    {
        ...
        [SerializeField] private int silver = 50;
        [SerializeField] private int gold = 50;
        [SerializeField] private string avatarUrl = "https://i.imgur.com/JQ9pRoD.png";

        public HelpBox _header = new()
        {
        ...
        private ObservableServerProfile CreateProfileInServer(string userId, IPeer clientPeer)
        {
            ...
            profile.Get<ObservableString>(ProfilePropertyOpCodes.displayName).Value =
                SimpleNameGenerator.Generate(Gender.Male);
        ...
        private void UpdateDisplayNameRequestHandler(IIncomingMessage message)
        {
            ...
            try
            {
                if (profilesList.TryGetValue(userExtension.UserId, out var profile))
                {
                    profile.Get<ObservableString>(ProfilePropertyOpCodes.displayName).Value =
        ...
        private void SellDemoItemMessageHandler(IIncomingMessage message)
        {
            try
            {
                    ...
                    && profile.TryGet(ProfilePropertyOpCodes.items, out ObservableDictStringInt items))
                {
                    ...
                    {
                        message.Respond($"Our store does not accept {data.Currency} as currency",
