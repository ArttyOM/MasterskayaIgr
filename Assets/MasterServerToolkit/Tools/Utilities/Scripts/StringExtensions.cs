...
namespace MasterServerToolkit.Extensions
{
    ...
    {
        private static readonly ConcurrentDictionary<uint, string> hashes = new();

        private const uint FNV_offset_basis = 0x01000193;
        private const uint FNV_prime = 0x811c9dc5;
        ...
        {
            var hash = (ushort) (FNV_1a_hash(value) & 0xFFFF);

            if (!hashes.ContainsKey(hash)) hashes[hash] = value;
