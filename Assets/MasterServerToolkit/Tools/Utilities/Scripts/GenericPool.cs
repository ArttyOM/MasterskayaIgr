...
namespace MasterServerToolkit.Utils
{
    public class GenericPool<T> where T : MonoBehaviour
    {
        ...
        {
            if (prefab == null) throw new NullReferenceException("Generic pool received a null as a prefab");

            prefab.gameObject.SetActive(false);
            _prefab = prefab;
            _freeObjects = new Stack<T>();

            if (!dontUseOriginal) Store(prefab);
        }
        ...
        {
            if (!_freeObjects.TryPop(out var obj))
                obj = InstantiateNew();
