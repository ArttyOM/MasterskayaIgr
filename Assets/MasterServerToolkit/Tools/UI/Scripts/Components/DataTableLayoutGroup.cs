...
namespace MasterServerToolkit.UI
{
    public class DataTableLayoutGroup : LayoutGroup
    {
        ...
        [SerializeField] private DataTableColInfo[] collsInfo;
        [SerializeField] private float cellSpacing = 0;
        [SerializeField] private float rowSpacing = 0;
        [SerializeField] private float minRowHeight = 30;
        ...
        private float CalculatedColWidth(int colIndex)
        {
            ...
            var totalCols = TotalCols();
            // Parent rect width
            var rectWidth = rectTransform.rect.size.x;

            // if we have one col
            if (totalCols == 1) return rectTransform.rect.size.x - padding.horizontal;
            ...
            if (totalCols > 1 && colIndex - 1 <= collsInfo.Length && collsInfo[colIndex].width <= 0)
            {
                var definedChildren = collsInfo.Where(ch => ch.width > 0).Count();
                var undefinedChildren = collsInfo.Where(ch => ch.width <= 0).Count();
                ...
                return (rectWidth - padding.horizontal - cellSpacing * undefinedChildren -
                        cellSpacing * (definedChildren - 1) - definedWidth) / undefinedChildren;
            }
        ...
        public override void CalculateLayoutInputVertical()
        {
            ...
            var minSpace = padding.vertical + rowSpacing * (totalRows - 1) + minRowHeight * totalRows;
        ...
        private void SetCellsAlongAxis(int axis)
        {
            ...
            for (var row = 0; row <= totalRows; row++)
            {
                ...
                for (var col = 0; col < totalCols; col++)
                {
                    ...
                    // Get current child
                    var childRect = rectChildren[currentCellIndex];
                    ...
                    if (axis == 0)
                    {
                        ...
                        if (totalCols <= 1)
                        {
                            childRect.sizeDelta = new Vector2(rectTransform.rect.size.x - padding.horizontal,
                                minRowHeight);
                        }
                        // If we have more than one col
                        else
                        {
                            var cellWidth = CalculatedColWidth(col);
                            childRect.sizeDelta = new Vector2(cellWidth, minRowHeight);
                        }

                        if (col == 0) xPos += padding.left;
