...
namespace MasterServerToolkit.UI
{
    public class ValidatableInputFieldComponent : ValidatableBaseComponent
    {
        ...
        {
            if (!currentInputField.interactable) return true;

            if (isRequired && string.IsNullOrEmpty(currentInputField.text.Trim()))
            {
                Logs.Error(string.IsNullOrEmpty(requiredErrorMessage)
            ...
            {
                Logs.Error(string.IsNullOrEmpty(regExpErrorMessage)
                    ? $"Field {name} is not match to RegExp pattern"
                    : regExpErrorMessage);
            ...
            {
                Logs.Error(string.IsNullOrEmpty(compareErrorMessage)
                    ? $"Field {name} is not equals to {compareToInputField.name}"
                    : compareErrorMessage);
