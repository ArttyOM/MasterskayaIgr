...
namespace MasterServerToolkit.UI
{
    ...
    {
        public enum UIPropertyValueFormat
        {
            F0,
            F1,
            F2,
        ...
        [Header("Settings")] [SerializeField] protected string id = "propertyId";
        ...
        [SerializeField] [Range(1f, 10f)] protected float progressSpeed = 1f;
        ...
        [SerializeField] protected bool useLable = true;
        [SerializeField] protected bool useIcon = true;
        [SerializeField] protected bool useProgress = true;
        [SerializeField] protected bool useColors = true;
        ...
        public Sprite Icon
        {
            get => iconImage != null ? iconImage.sprite : null;
            set
            {
                if (iconImage)
        ...
        private void OnValidate()
        {
            ...
            if (valueText) valueText.gameObject.SetActive(useValue);

            if (lableText) lableText.gameObject.SetActive(useLable);

            if (iconImage) iconImage.gameObject.SetActive(useIcon);
        ...
        private void Update()
        {
            if (minValue < maxValue)
            {
                if (smoothValue && Application.isPlaying)
                    currentProgressValue = Mathf.Lerp(currentProgressValue, targetProgressValue,
                ...
                if (useColors && iconImage) iconImage.color = Color.Lerp(minColor, maxColor, currentProgressValue);
