...
namespace MasterServerToolkit.Networking
{
    public class WsClientSocket : BaseClientSocket, IClientSocket, IUpdatable
    {
        ...
        private readonly Dictionary<ushort, IPacketHandler> handlers = new();
        ...
        private void OnMessageReceivedHandler(IIncomingMessage message)
        {
            try
            {
                if (handlers.TryGetValue(message.OpCode, out var handler))
                {
                    if (handler != null)
                        handler.Handle(message);
                    else
                        Logs.Error(
                            $"Connection is missing a handler. OpCode: {Extensions.StringExtensions.FromHash(message.OpCode)}");
                }
            ...
            {
                Logs.Error(
                    $"Failed to handle a message. OpCode: {Extensions.StringExtensions.FromHash(message.OpCode)}, Error: {e}");
        ...
        public void WaitForConnection(ConnectionDelegate connectionCallback, float timeoutSeconds)
        {
            ...
            void onConnected(IClientSocket client)
            {
                ...
                if (!timedOut) connectionCallback.Invoke(client);
            }
        ...
        public void AddConnectionOpenListener(ConnectionDelegate callback, bool invokeInstantlyIfConnected = true)
        {
            ...
            if (IsConnected && invokeInstantlyIfConnected) OnConnectionOpenEvent.Invoke(this);
        }
        ...
        public void AddConnectionCloseListener(ConnectionDelegate callback, bool invokeInstantlyIfDisconnected = true)
        {
            ...
            if (!IsConnected && invokeInstantlyIfDisconnected) OnConnectionCloseEvent.Invoke(this);
        }
        ...
        public void RemoveMessageHandler(IPacketHandler handler)
        {
            ...
            if (handlers.TryGetValue(handler.OpCode, out var previousHandler) && previousHandler != handler) return;
