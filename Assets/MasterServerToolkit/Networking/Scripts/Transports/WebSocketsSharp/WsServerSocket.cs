    ...
    public class WsServerSocket : IServerSocket
    {
        ...
        public void Listen(string address, int port)
        {
            try
            {
                ...
                {
                    var url = $"{(UseSecure ? "wss://" : "ws://")}{address}:{port}";
                ...
                if (UseSecure)
                {
                    ...
                    {
                        logger.Error(
                            "You are using secure connection, but no path to certificate defined. Stop connection process.");
                        return;
                    }
        ...
        private void SetupService(WebSocketServer server)
        {
            ...
            server.AddWebSocketService<WsService>($"/app/{ApplicationKey}", (serviceForPeer) =>
            {
                ...
                peer.OnConnectionOpenEvent += (peer) => { OnPeerConnectedEvent?.Invoke(peer); };

                peer.OnConnectionCloseEvent += (peer) => { OnPeerDisconnectedEvent?.Invoke(peer); };
            });
