...
namespace MasterServerToolkit.Networking
{
    ...
    {
        private readonly ConcurrentDictionary<int, ResponseCallback> acknowledgements = new();
        protected readonly List<long[]> ackTimeoutQueue;
        private readonly ConcurrentDictionary<uint, object> peerPropertyData = new();
        private int _id = -1;
        ...
        private static readonly object idGenerationLock = new();
        ...
        protected BasePeer()
        {
            ...
            timeoutMessage = new IncomingMessage("-1".ToUint16Hash(), 0, "Time out".ToBytes(),
        ...
        {
            peerPropertyData.TryGetValue(id, out var value);
            return value;
        }
        ...
        {
            if (acknowledgements.TryRemove(ackId, out var ackCallback))
        ...
        public void HandleDataReceived(byte[] buffer, int start)
        {
            ...
            var message = MessageHelper.FromBytes(buffer, start, this);
        ...
        protected virtual void Dispose(bool disposing)
        {
            if (!disposedValue)
            {
                if (disposing) MstTimer.OnTickEvent -= HandleAckDisposalTick;
