...
namespace MasterServerToolkit.Networking
{
    ...
    public static class SerializationExtensions
    {
        ...
        public static byte[] ToBytes(this IEnumerable<ISerializablePacket> list)
        {
            ...
            using (var ms = new MemoryStream())
            {
                using (var writer = new EndianBinaryWriter(EndianBitConverter.Big, ms))
                {
                    ...
                    foreach (var item in list) item.ToBinaryWriter(writer);
                }
        ...
        /// <returns></returns>
        public static List<ISerializablePacket> FromBytes(this List<ISerializablePacket> list, byte[] data,
            Func<ISerializablePacket> factory)
        {
            using (var ms = new MemoryStream(data))
            {
                using (var reader = new EndianBinaryReader(EndianBitConverter.Big, ms))
        ...
        public static Dictionary<string, string> ReadDictionary(this EndianBinaryReader reader)
        {
            ...
            if (length > 0) return new Dictionary<string, string>().FromBytes(reader.ReadBytes(length));
        ...
        /// <returns></returns>
        public static string ToReadableString(this Dictionary<string, string> dictionary, string itemsSeparator = ";",
            string kvpSeparator = ":")
        {
            var readableString = string.Empty;

            if (dictionary != null && dictionary.Count > 0)
                readableString = string.Join(itemsSeparator,
                    dictionary.Select(p =>
                            p.Key + $"{kvpSeparator}" + (!string.IsNullOrEmpty(p.Value) ? p.Value : string.Empty))
                        .ToArray());
        ...
        /// <returns></returns>
        public static Dictionary<string, string> FromReadableString(this Dictionary<string, string> dictionary,
            string value, string itemsSplitter = ";", string kvpSplitter = ":")
        {
            dictionary.Clear();

            var kvps = value.Split(itemsSplitter, StringSplitOptions.RemoveEmptyEntries);

            foreach (var kvp in kvps)
            {
                var splitterIndex = kvp.IndexOf(kvpSplitter);
                var dicKey = kvp.Substring(0, kvp.IndexOf(kvpSplitter));
                var dicValue = kvp.Substring(splitterIndex + 1);
