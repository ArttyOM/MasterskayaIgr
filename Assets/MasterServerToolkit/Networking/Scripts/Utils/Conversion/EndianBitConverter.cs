...
namespace MasterServerToolkit.Networking
{
    ...
    public abstract class EndianBitConverter
    {
        ...
        private static readonly LittleEndianBitConverter little = new();
        ...
        private static readonly BigEndianBitConverter big = new();
        ...
        public decimal ToDecimal(byte[] value, int startIndex)
        {
            ...
            var parts = new int[4];
            for (var i = 0; i < 4; i++) parts[i] = ToInt32(value, startIndex + i * 4);
        ...
        public byte[] GetBytes(decimal value)
        {
            ...
            for (var i = 0; i < 4; i++) CopyBytesImpl(parts[i], 4, bytes, i * 4);
        ...
        {
            var parts = decimal.GetBits(value);
            for (var i = 0; i < 4; i++) CopyBytesImpl(parts[i], 4, buffer, i * 4 + index);
        }
        ...
        {
            if (buffer == null) throw new ArgumentNullException("buffer", "Byte array must not be null");

            if (buffer.Length < index + bytes) throw new ArgumentOutOfRangeException("Buffer not big enough for value");
