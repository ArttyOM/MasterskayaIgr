    ...
    public class GeometryCollider : MonoBehaviour
    {
        ...
        public static void Bake(GameObject go, bool forced)
        {
            ...
            if (spriteShapeController != null && polyCollider != null)
            {
                ...
                {
                    var splineHashCode = spline.GetHashCode();
                ...
                var indexArrayLocal = new NativeArray<ushort>(indexArray.Length, Allocator.Temp);

                var points = new List<Vector2>();
                int indexCount = 0, vertexCount = 0, counter = 0;
                for (var u = 0; u < geomArray.Length; ++u)
                    if (geomArray[u].indexCount > 0)
                    {
                        for (var i = 0; i < geomArray[u].indexCount; ++i)
                        {
        ...
        private static void OuterEdges(PolygonCollider2D polygonCollider, NativeArray<ushort> triangles,
            NativeSlice<Vector3> vertices, int triangleCount)
        {
