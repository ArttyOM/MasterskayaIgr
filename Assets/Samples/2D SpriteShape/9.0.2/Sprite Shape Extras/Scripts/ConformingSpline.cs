...
namespace SpriteShapeExtras
{
    ...
    public class ConformingSpline : MonoBehaviour
    {
        ...
        private void Update()
        {
            if (m_ParentObject != null) hashCode = CopySpline(m_ParentObject, gameObject, hashCode);
        }
        ...
        private static int CopySpline(GameObject src, GameObject dst, int hashCode)
        {
            ...
            if (parentSpriteShapeController != null && mirrorSpriteShapeController != null &&
                parentSpriteShapeController.spline.GetHashCode() != hashCode)
            {
                var srcController = new SerializedObject(parentSpriteShapeController);
                var dstController = new SerializedObject(mirrorSpriteShapeController);
                var srcSpline = srcController.FindProperty("m_Spline");
